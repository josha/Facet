...
| `pixel_icon_chevron_left.png` | 32×32 | 8×8 | — | near-white glyph | `icons["chevron.leading"]` |
| `pixel_icon_chevron_up.png` | 32×32 | 8×8 | — | near-white glyph | `icons["chevron.up"]` |
| `pixel_icon_chevron_up_down.png` | 32×32 | 8×8 | — | near-white glyph | `icons["chevron.up.chevron.down"]` |
| `pixel_icon_more.png` | 32×32 | 8×8 | — | near-white glyph | `icons["more"]` |
| `pixel_icon_edit.png` | 32×32 | 8×8 | — | near-white glyph | `icons["edit"]` |
| `pixel_icon_radio_off.png` | 32×32 | 8×8 | — | near-white glyph | `icons["facet:selection.radio.off"]` |
| `pixel_icon_radio_on.png` | 32×32 | 8×8 | — | near-white glyph | `icons["facet:selection.radio.on"]` |
| `pixel_icon_check_off.png` | 32×32 | 8×8 | — | near-white glyph | `icons["facet:selection.check.off"]` |
...
9. **The icon set is COMPLETE, and that is what licenses the opt-out** (added
   2026-09-11). This package sets `identity.standardIcons = false`, which declines
   Facet's own icon art — correctly, because a 128 px anti-aliased silhouette
   under `ResampleMode = Pixelated` is nearest-neighbour mush on a 4 px grid. The
   opt-out is only honest while every name a control asks for resolves HERE:
   before this, eight names had no art in any rung and fell through to ASCII
   characters, which a player photographed in Studio (`^v` in a picker trigger,
   `*` and `o` for a radio). The eight added glyphs — the leading and up
   chevrons, the stacked pop-up pair, `more`, `edit` and the three selection
   marks — close the set. `tests/icon_coverage.spec.luau` fails this package if a
   hole ever reopens.
...
returned content IDs in `upload-manifest.json`. The original twenty went up
through Studio with the ids hand-transcribed; since 2026-09-11 the headless route
covers a theme package too, and it is the one to use:

```sh
python3 tools/upload_icons.py --theme pixel-quest --registry examples/themes/pixel_quest.luau
```

It uploads every PNG the manifest has no content id for, polls each Open Cloud
operation to `done`, writes the ids into both the manifest and the Luau module,
and leaves every authored field (`slot`, `designPx`, the slice geometry) alone —
those are the author's, not the tool's. `--recheck` re-reads moderation state and
asset type afterwards; the eight added on 2026-09-11 came back **Approved** and
`Image`. Another project reproducing the package uploads the same PNGs under its
own account and substitutes its IDs — no hidden assets.
