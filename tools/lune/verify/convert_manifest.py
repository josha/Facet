...
#[[ THE TREES THAT ARE LEAVING THE PUBLIC TIP (pre-archival pass, 2026-08-30).
#
#   `artifacts/`, `docs/plans/`, `docs/handoff/`, `docs/research/`,
#   `docs/superpowers/`, `.superpowers/` and `ui_todo.md` are the stage record.
#   The director archives them and deletes them from the tip, so NO row may read
#   them: a row that did would go from green to broken on the day of the
#   deletion, which is the definition of a check that is about to stop meaning
#   anything.
#
#   Two exceptions, both because the file is REGENERATED by a producer in this
#   same graph rather than being a record of anything:
#     * the conformance scorecards, written by the corpus CLI in the same clause
#       that reads them;
#     * `artifacts/bench.json`, `artifacts/test.json`, `artifacts/boundary.json`
#       and everything under `artifacts/verify/`, each written by its producer.
#   Those survive the deletion because the run recreates them. ]]
ARCHIVAL_TREE = (
...
ARTIFACT_PATH = re.compile(r"(?:" + ARCHIVAL_TREE + r")[A-Za-z0-9._/${}-]*")


def archival_paths(clause: str) -> list[str]:
    """Paths in this clause that name a tree the director is about to delete."""
    out = []
    for m in ARTIFACT_PATH.finditer(clause):
        text = m.group(0)
        if REGENERATED.match(text):
            continue
        # `'artifacts/' not in l` inside a python one-liner is this converter's
        # own guard-import idiom quoting a prefix, not a file being read.
        after = clause[m.end(): m.end() + 12]
        if after.startswith("' not in") or after.startswith("\\' not in"):
            continue
        out.append(text)
    return out
...
# The trees that STAY. `docs/` is enumerated rather than taken whole, because
# three of its subtrees are leaving with the stage record.
SOURCE_PATH = re.compile(
    r"(?<![\w/])(?:src|tests|examples|tools|bench|package|skills)/"
    r"|docs/(?:guide|reference|extending|MAINTAINERS|INVENTORY|DECISIONS)"
)
...
def main() -> int:
    ...
    for phase in phases:
        for entry in manifest.get(phase, []):
            ...
            #   `f=artifacts/x && grep -qF "y" "$f"` splits into a moved
            #   assignment and a kept grep — and the kept grep then reads an
            #   UNSET variable, which passes. That is a check that cannot fail,
            #   manufactured by the split itself.
            #
            #   The answer is to MOVE the offending clause too, not to abandon
            #   the split: a clause whose subject is a file in a tree that is
            #   about to be deleted belongs with that file's content hash,
            #   whatever else it mentions. Repeated until nothing kept reads a
            #   moved variable. ]]
            while True:
                offenders = [c for c in kept if any(f"${v}" in c for v in moved_assigns)]
                if not offenders:
                    break
                for c in offenders:
                    kept.remove(c)
            ...
            #[[ A RECEIPT IS FOR RECORDED MACHINE EVIDENCE. A PIN ON A LEDGER
            #   IS A RECORD, AND RECORDS ARE ARCHIVED (director ruling).
            #
            #   The distinction is what the file IS, not where it lives. A Studio
            #   drive, a device capture, a performance capture, an
            #   engine-feasibility probe or a measured row is evidence a machine
            #   took and a headless run can never re-take: it keeps its row and
            #   gets a content hash, and the private archive holds the file with
            #   the same checksum. A stage's acceptance table, review packet or
            #   decision text is a RECORD of a decision that has been made: a
            #   hash of it would be a pin nobody can ever verify once the
            #   director deletes the tree, so it is archived instead, with a
            #   coverage-map row naming what it carried. ]]
            receipt_path = None
            if artifact_paths:
                living = [q for q in artifact_paths if LIVING_EVIDENCE.match(q)]
                records = [q for q in artifact_paths if q not in living]
                if records:
                    dropped_records.append({
                        "row": row_id, "phase": public_phase, "name": name,
                        "requirements": base["requirements"], "paths": records,
                    })
                if living:
                    joined = " ".join(living)
                    cls_name = (
                    ...
                    receipt = {
                        ...
                        "summary": f"{name}: {len(items)} recorded {cls_name} file(s), pinned by content hash.",
                        "evidence": items,
                    }
                ...
                if not receipt_path and not shell and not row_producers and not row_ids and not stdout_pins:
                    historical.append({"row": row_id, "phase": public_phase, "name": name,
...
def write_coverage(path: str, graph: dict, historical: list, receipts: int, manifest_reqs: set,
                   internal_clauses: list, dropped_records: list) -> None:
    ...
    if dropped_records:
        n = sum(len(d["paths"]) for d in dropped_records)
        L += [
            "",
            f"## 5c. Pins on a closed stage's record, archived ({len(dropped_records)} rows, {n} files)",
            "",
        ...
        for d in dropped_records:
            paths = ", ".join(f"`{q}`" for q in d["paths"][:4])
            if len(d["paths"]) > 4:
