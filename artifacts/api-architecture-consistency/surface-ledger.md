...
| Dim-shape shorthands: `UI.fill` `UI.hug` | value producers (not blueprint modifiers — no `bp` argument) | returns the `dim` table a `width`/`height` prop already accepted raw (framework-gaps-phase2 in-brief item 14); `fill(weight?)`, `hug({min?,max?})` | ADR-0049 | blueprints |
...
| Item | Kind | Pattern / exception | Dispositions | Fragment |
|---|---|---|---|---|
| `Controls` · `Controls.Table` · `Controls.Slider` · `Controls.Stepper` · `Controls.Picker` · `Controls.PopupButton` · `Controls.Menu` · `Controls.TabView` · `Controls.Label` · `Controls.Chip` · `Controls.Rating` · `Controls.TextInput` · `Controls.ProgressView` · `Controls.LevelPicker` · `Controls.DisclosureGroup` · `Controls.VirtualList` · `Controls.VirtualGrid` · `Controls.RowActions` · `Controls.Callout` · `Controls.AsyncImage` | namespace of composite factories | ADR-0037 (0.10.0): one frozen table, nineteen typed entries, each `(core, spec)`. Each entry is a CLOSURE over the library around the same `build` the row below it names — not a second implementation — so every classification, pattern and disposition recorded for `new<Name>` applies unchanged to `Controls.<Name>`. The nineteen `new<Name>` rows below stay live and are declared in `DEPRECATIONS` (since 0.10.0, removeNoEarlierThan 0.12.0) | ADR-0037; tests/controls_namespace.spec.luau; tools/check_call_shape_drift.py | controls |
| `preload` | loading-screen seam over the `Controls` namespace | `() -> number`, no arguments, no state. Four of the nineteen entries above defer their `require` into the closure that builds them — the four that declare no `Spec` type and therefore need nothing from their module at load (228 KB [131..313] of Lua heap, requalification.md §7). `preload()` force-loads exactly those four for a game that would rather pay at the loading screen than on the frame a player first opens a list, and returns how many it loaded so a caller can log the cost. Idempotent by `require`'s own cache; calling it never is the supported default | ADR-0037 (wave T15); tests/virtualization.spec.luau (the deferred set, the construction seam, and preload's own set); tools/check_types.py (why only four) | controls |
...
| `newVirtualList` | composite | control-build, flat variant (pre-rule) | F-25 (dump proof), DOC-11, PKT-1; ADR-0043 (2026-08-22) added two CONDITIONAL fields to the returned table — `viewportExtent` and `crossExtent`, each a `Readable<number>` published ONLY when the list was asked to measure that axis (`viewportExtent = "auto"` / `crossExtent = "hug"\|"measured"`). A list that declared its own numbers publishes neither, so no existing caller's returned table gained a key; the classification is unchanged because both are reads of the control's own state, exactly as `scrollTop` and `focusedKey` are | controls |
| `newVirtualGrid` | composite | control-build, flat variant (matches its `newVirtualList` sibling deliberately — the two share the `virtual_extents` index and are read against each other) | ADR-0043 (2026-08-22) added the same conditional `viewportExtent` `Readable`, on the same terms; `crossExtent` is deliberately NOT offered here (a grid's lane width is derived from its cross extent, so hugging would be circular) | controls |
...
| `newCallout` | composite | control-build, `api` variant; the app-PUSHED coach mark (navigation-and-menus D3b). Presents on `presenter.presentCallout`, which is a one-at-a-time driver over the shipped toast scheduler. Its one unusual obligation is a REQUIRED `onRetire(reason)`: persistence is the caller's, so the construct reads `seen`/`sessions`/`featureUsed` Readables and reports back rather than writing anything | none (clean); NOTE (D3b, postdates the Step 7 F-n audit round) | controls |
...
| `newTabView` | composite | control-build; adaptive PLACEMENT (`adaptive.navPlacement`) rather than an adaptive presentation. Composes `newPicker` for its strip (navigation-and-menus D5) — no second option row — and adds the three things a picker never owns: the content subtree's lifecycle (lazy, evicted on switch, really disposed through `UI.When`'s own branch scope), the placement that restructures the screen, and NESTING (an inner TabView never claims the app-level placement; `dump().placementSource` reports which authority answered). Returns `placement` beside `blueprint`/`dump`/`dispose` so a screen with its own chrome reads one answer instead of re-deriving the policy. ADR-0045 (2026-08-22) added the CHROME the placement moves, as four optional spec keys — `accessories` ({ head?, foot?, trailing?, aboveBar? }, each a `(placement, scope) -> Blueprint?` factory invoked only by the homes that host it), `railWidth`, a pass-through `textSize` and a content-branch `transition` — plus one MODULE-LEVEL pure export beside `resolvePlacement`/`resolveSizing`: `tab_view.accessoryPlacements(slot) -> {string}?`, the hosting matrix as data, returned as a copy. Classification unchanged: every key is optional with a nil default and the no-accessory mounted tree is byte-identical, which is what let `p4_foyer` keep working with no edit | none (clean); DOC-22, PKT-6 inherited from the strip it composes; NOTE (D5, postdates the Step 7 F-n audit round); ADR-0045 | controls |
...
| `adaptive` · `adaptive.sizeClass` · `adaptive.heightClass` · `adaptive.navPlacement` · `adaptive.orientationFor` · `adaptive.axisFor` · `adaptive.columnsFor` · `adaptive.cardsPerView` · `adaptive.cardPeek` · `adaptive.conditions` · `adaptive.sizeClassAtLeast` · `adaptive.BREAKPOINTS` · `adaptive.HEIGHT_BREAKPOINTS` · `adaptive.CARD_MIN_WIDTH` · `adaptive.CARD_PEEK` · `adaptive.LANE_MEASURE` · `adaptive.laneMeasureFor` · `adaptive.DEFAULT_STACK_ABOVE` · `adaptive.fitsIn` · `adaptive.fits` | pure decision module + reactive half | value-first pure fns; `opts.scope` ownership. **`fitsIn`/`fits` (2026-08-22, framework-gaps-phase2 audit §2b) are the CONTAINER-relative half** — every other entry here is viewport-relative, and `UI.ViewThatFits` answers the same question with a SUBTREE, so a consumer that needs the fit DECISION as a value (reserve space in a sibling, place or skip a minimap, choose between two surfaces before either is built) had nothing to call. `fitsIn(needs, container, clearance?)` is the pure half and is the LADDER'S OWN PREDICATE — `chosenCandidate` tests `availW == math.huge or w <= availW` and so does this, driven side by side across a width sweep in `tests/fits_ideal_size.spec.luau`; `fits(core, spec)` is the reactive half, and its `needs` takes a LIST of parts rather than a pre-computed sum because naming the parts is what lets them be the same reads the content is built from (a sum computed once cannot ride the ten-foot ladder, and a part that arrives nil makes a sum SMALLER and the verdict more permissive — refused here, at construction for a static part and on the core's error channel for a live one) | F-28/F-29 (Readable typing, scope in type), DEP (`contentWidth`); ADR-0044 | services |
| `adaptive.conditions()`'s return, GAP 7 additions (2026-08-22, framework-gaps-phase2 audit §7) — not separately-classified STATIC members (`check_surface_ledger` walks `Facet`'s static shape, not a call result; the return type is the classification unit) | pure decision module + reactive half, same row as `adaptive` above | **`navSidebar`/`navTopBar`/`navBottomBar`/`navBottomBarCompact`** — the four homes `adaptive.navPlacement` names, PROMOTED off the equality-check idiom `src/controls/tab_view.luau`'s private `isPlacement(name)` already used, so a caller never hand-rolls a `navPlacement == "sidebar"` memo again (audit-cited sites: `p1_glade`, `p3_sipworks/views/shell.luau`). **`isCompactOnly`/`isRegularOrWider`/`atLeast(target)`** — the `isRegular` TRAP: that name reads "at least regular" and behaves "the middle class only" (false on `wide`, the widest screen there is); three of five reference apps hand-wrote `not use(isCompact)` for the question they meant. `isCompactOnly` is `isCompact` itself, under the pairing name (same memo, not a second fact); `isRegularOrWider` is `atLeast("regular")`; `atLeast` is the general ranked form, backed by the new pure `adaptive.sizeClassAtLeast`. **Teaches-wrong #7 (raw `conditions.viewportWidth`) does NOT dissolve here** — its two live sites are both genuine pixel arithmetic no boolean class predicate answers: `examples/gallery/scenarios/composition.luau:67` clamps a fixed px offer against the raw viewport extent (`math.min(px, have)`); `examples/reference/p3_sipworks/init.luau:300` (`app.paneSplit`) compares it against an authored METRIC threshold (`use(app.cond.viewportWidth) >= app.metric("twoPaneMin", use)`) — "can two panes stand beside each other" is its own fact, not the same question as a size class (the site's own comment: a sidebar has already taken width out of the window, so `sizeClass` and this threshold answer differently near 600px); it stays open | task-g7g11-report.md | services |
...
| `layout` · `layout.transformFootprint` | pure geometry module | value-first pure fn, no core/engine/theme dependency | framework-gaps-phase2 gap 33 (audit-marked "teaches-wrong 12"): the axis-aligned reserved-box formula for a `w x h` node scaled/rotated by `(scale, deg)`, published so a consumer computes the reservation `docs/reference/api.md`'s `scale` row already documents in prose instead of hand-transcribing the trigonometry (`examples/gallery/scenarios/nested_compositing.luau`'s former `turnedFootprint`, which now calls this) | services |
| `text` · `text.measure` · `text.fit` · `text.size` · `text.facts` · `text.lineBox` · `text.AVG_GLYPH_FRACTION` | pure decision module (module-global caches noted) | spec-table canonical; E-15 (positional measure) | F-31, F-32, DOC-2; `AVG_GLYPH_FRACTION` (2026-08-22, framework-gaps-phase2 gap 11) is the fallback fraction as DATA, so a caller already holding this table can source it instead of hand-copying it — RascalRally's `HudZoneModel.fitSize` did, as `GLYPH_EM = 0.62`, under a comment misidentifying it as "Gotham Bold cap advance". `src/measure.luau` (outside the `Facet` table by design, so not itself ledger-eligible — see the `adaptive.conditions()` row above and `tag_sync.luau`'s precedent) republishes the same `text_metrics` module, and its `.AVG_GLYPH_FRACTION` field is the identical value reached with no `Facet` table at all | services |
...
| `themes` · `themes.define` · `themes.resolve` · `themes.neutral` · `themes.neutralPackage` · `themes.metricScale` · `themes.forDisplay` · `themes.paintForDisplay` · `themes.baseOf` · `themes.densityClassOf` · `themes.declareApp` · `themes.appMetrics` · `themes.lintProperty` · `themes.checkCoverage` · `themes.SCHEMA` | curated namespace | strongest docs-coverage story (per-member check) | PKT-8 (types), NOTE (three return conventions — SEAM-13, constitution §6 rules future); `paintForDisplay` joined the namespace in `c4d0591` (ADR-0040 row B-17, director 2026-08-21) as `forDisplay`'s paint-side twin — one derivation for radii/strokes, spent by `sheet_model`, `screen_target` and `theme_controller`; `declareApp`/`appMetrics` joined for ADR-0042 (framework-gaps-phase2 gap 1) — the reserved `app.*` namespace an application declares once so its own structural numbers are spellable metric names, theme-movable (`metrics.app`) and on the ten-foot ladder. `declareApp` is the ONE process-wide mutable fact in the pure theme half, and it is there because `isMetricPath` is a construction-time check with no snapshot in scope. Its two companions on the module — `snapshot.observeAppDeclarations` and `snapshot.appGeneration` — are deliberately INTERNAL and NOT on this namespace: they are the reactive edge `env/environment.luau` registers so its metric memo re-runs on a declaration, the registry is WEAK (an observer keeping no reference of its own would have its edge collected with no diagnostic), and the only safe holders are the environment itself and a caller retaining the `unsubscribe` it returns | seams |
