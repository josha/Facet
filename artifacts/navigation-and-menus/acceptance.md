...
| ID | User-visible behaviour | Risk | Evidence | Driver | Artifact | Status |
|---|---|---|---|---|---|---|
| **NM-8.1** | Tapping a sortable header cycles an owner-held `sortOrder`; filter and sort compose over one source list | Sorting the filtered list and filtering the sorted list differ the moment both are active | E1 | spec over both active | TBD | PENDING |
| **NM-8.2** | A manual reorder clears the sort (the iTunes rule) | Leaving it undefined means a drag silently re-sorts itself away | E1 | spec | TBD | PENDING |
| **NM-8.3** | Name and Artist resize through all three routes; Rating **refuses** | Pinning the sole `fill` column leaves nothing to absorb the remainder and the row under-fills | E1 | three-route specs re-pointed off `table_columns` | TBD | PENDING |
| **NM-8.4** | Three columns fit at **320×640 in edit mode** with explicit `minWidth`s, re-measured — or the fixture falls back to two and says so | A third fixed 70 px column left the Name cell 6 px there; "a `fill` column is different" is a hypothesis until measured | E1 | measurement at 320×640 | TBD | PENDING |
| **NM-8.5** | The live width readout and the selected-column hint survive the merge | `api.selectedColumn` paints nothing, so after a gamepad Activate the stick silently means something new | E1 | spec | TBD | PENDING |
| **NM-8.6** | ~700 lines of `table_columns` spec are green **on the playlist fixture before** the scenario is deleted | Deleting the fixture first is the named failure mode | E1 | suite counts before/after | TBD | PENDING |
