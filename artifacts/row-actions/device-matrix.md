...
| Vertical-pan Activate (does the engine cancel it?) | **Answer this one first — a code decision waits on it.** On a real touch device, pan the list vertically *starting on a row* and release: does that row's `onActivate` fire? Headless it DOES — measured 2026-08-13 on the clean-room hosted list (`tests/virtual_list_row_actions.spec.luau`'s `world` + `dragDown` + `releaseActivating`, 40px rows): **1 stray activation in all four combinations**, `dy=20` (release still inside the row rect) and `dy=90`, both release orders, `engagedKey=nil` and no tray action fired. `hostedResolveAxis` arms nothing when a gesture resolves vertical (`pending.declined = true`), so nothing swallows that `Activated`. Unchanged by `79649d6`; pre-existing, **deliberately not fixed** because the fix rests on the unverified premise that a native `ScrollingFrame` drag cancels its child button's `Activated` — the very premise the HORIZONTAL branch refuses to rely on. If the device says the engine cancels it, the headless stray is a harness artefact and adding an arm would eat real taps; if it does not, the arm belongs at the vertical resolve. Booked in `docs/plans/row-actions-hosted-mode-plan.md` (device rider 5) and `docs/plans/swiftui-parity-round3-brief.md` (E2). |
