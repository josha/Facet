...
The DIR wave closed clean (review + mutation-verified fix round); device confirmations ride the batched pass. DIR2 (expand live round): DIR2-1 empty pills FIXED (cover retired structurally), DIR2-3 X icon FIXED, plate width FIXED; DIR2-2 base-disappear cause removed + fence, OPEN pending the device click (owner: device packet). DIR3 (round 3): DIR3-1 transient-over-screen framework rule + DIR3-2 chrome shoulder access — landed 48b6e7b (suite 6864), review seat running. Live check on the console row: L1 opens the panel OVER the live screen (10400/scrim 10350/screen 10300) ✓; **R1 does nothing live** (headless-green): presenter.luau binds ButtonL1/R1 as Adjust step keys for focused adjustTargets — the chrome comment "no other context binds either key" is false. DIR4 (round 4, 2026-08-21, director live): chips vanish on real console — chrome mounts edge-to-edge at (12,12) while the body honors effectiveOverscanInsets (x=106): the TV bezel eats the chips; plus no on-screen affordance teaches the L1/R1 shoulders. DIR4 round dispatched: overscan-aware chrome, shoulder glyphs under gamepad, R1 contention fix. ButtonB is CoreGui-permanently-bound — injection refuses it, so B-close is device-half evidence forever. The published-place question is resolved: the phone build may predate the 2026-08-15
