...
| id | question | measured today | expected paradigm | verdict |
|---|---|---|---|---|
| **B-1** | `UI.Grid{}` with neither `columns` nor `minColumnWidth` | **one lane at every combo, 390 through 1920.** Probe 2 · L-B1: six 160×100 cards stacked in a single column at all six combos. Source: `grid.luau:174-193` `gridLaneCount` returns `1` when both are absent | the most-reached-for card container should lane itself from the space it got. A one-lane default on a 1920 TV is a column of cards down the left edge | **WRONG** (ADAPT-L2; Part 1's ADAPT-22, still open) — **RE-VERDICTED 2026-08-21: RIGHT.** Declaring neither key now MEANS `minColumnWidth = "intrinsic"`, so the same six cards measure 2/4/6/9/5/7 lanes at the six combos — the B-2 row exactly, television under desktop (wave LAYOUT-FIX) |
...
**Re-verdict note, appended 2026-08-21 (wave LAYOUT-FIX).** The table below is the
...
| **ADAPT-L2** — annotation, appended 2026-08-21 | **FIXED by wave LAYOUT-FIX.** The default is the route the guide already taught: `minColumnWidth = "intrinsic"`, reaching `adaptive.columnsFor` through the same line the declared route reaches it through, so the ten-foot cap rides along and no new number enters the framework. The second half of this row — 0 of 35 sites bind lanes — is answered rather than fixed: there is nothing left to bind, and a re-census of all 35 found no site hand-computing lanes to dodge the old default, so the blast radius was zero shipped call sites. See the LAYOUT-FIX addendum in `fixes.md` | — |
...
| **ADAPT-L4** — annotation, appended 2026-08-21 | **FIXED by wave LAYOUT-FIX**, by this row's own smallest fix: `axis` is `required = true`, and the refusal names the fact (`Facet.adaptive.conditions(core, env).axis`), the host that publishes it with no wiring (`Facet.client.host.new`) and both unconditional classes. The three fixtures bind the surface's own axis, so their comments are true — measured as a column at 320 and a row at 1280. Two registries the change touched were found by the suite rather than by reading: `check_primitives` needs a MINIMAL_PROPS entry for any required prop, and `stack_distribution.spec`'s class-set case was constructing a bare one | — |
