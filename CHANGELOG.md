...
numbers follow the [versioning policy](CONTRIBUTING.md#6-versioning). Before 1.0,
a minor version may change public behavior.
...
Facet draws client-side Roblox interfaces from a plain-data description of a
screen. It supplies composite controls, a layout solver, focus and semantic
input, adaptive facts, theme packages, virtual collections, region transitions
and a motion runtime. Layout, focus, state and teardown run in Luau and are
testable without a Roblox process. A solved screen lands on a `ScreenGui`, on a
billboard that follows an object, or on a world-fixed `SurfaceGui`.

**One authoring model.** `local app = Facet.new(opts)` builds the application and
`local UI = app.controls` is its constructor table. A component is a plain Luau
function that returns a node. State is `Compose.cell`, `Compose.formula`,
`Compose.watch` and `Compose.cleanup`, and lifetime is a Compose owner. Present
with `app.mount`, `app.presentModal`, `app.presentAnchored` and
`app.presentToast`.

**Breaking:** `Facet.UI`, `Facet.View`, `Facet.component`,
`Facet.Controls.<Name>(core, spec)`, `Facet.mount`, `Facet.newCore`,
`Facet.newPresenter` and `Facet.preload` are removed.
...
Not published, so its deprecation window begins at its first release. Until then the register below
