...
| Conformance extension | `TextField`/`TextInput` rows in `src/class_contract.luau` + the registration checker; the §10.2 control contract (build/render, per-input interaction, no factory reruns, dump determinism, registry neutrality) encoded as specs. |
