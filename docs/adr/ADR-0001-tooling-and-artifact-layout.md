...
- **Decision:**
  - Commands live in `GameStudio/ui/Facet/tools/` as `<name>.sh` wrappers (doctor.sh, test.sh, gate.sh, bench.sh, render.sh, interact.sh, soak.sh, decision.sh) over Lune implementations in `tools/lune/`. `run-tests.sh` stays and `tools/test.sh` delegates to it.
  - Artifacts write to `GameStudio/ui/Facet/artifacts/phase-<N>/` (stable, versioned by filename, machine-readable JSON preferred). The canonical aggregate gate is `tools/gate.sh`, which writes `artifacts/phase-<N>/gate.json` for the active phase and exits 0 only on PASS.
