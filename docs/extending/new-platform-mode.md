...
|---|---|---|---|
| Contracts | Can spatial input and world surfaces be added without rewriting screens? | `presentationSpace` fact + `presentationProfile` policy; `Facet.spatial` optional event payload; `capabilities.spatialPointer`; `target_contract.FUTURE.surface`, now shipped as `surface_target` (ADR-0063) — a FLAT world surface, which is not a spatial claim | **Shipped** (contracts, plus one flat world target) |
...
| Occlusion | What does a control do when its surface is behind geometry, edge-on, or off-view? | measured for a FLAT surface under an ordinary pointer (ADR-0063): geometry in front blocks input, and `AlwaysOnTop = true` defeats that, which is why `surface_target` pins it false. Undefined for a spatial pointer, which is what this row asks | PENDING_PHYSICAL |
...
| Performance | What does a world surface cost per frame at production node counts on the supported hardware? | a world target now exists (`surface_target`, ADR-0063) and its per-frame cost is one of its own named open questions; nothing has been measured on spatial hardware | PENDING_PHYSICAL |
