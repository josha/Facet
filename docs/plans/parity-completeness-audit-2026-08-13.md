...
3. **Genuinely peripheral to a game UI (19 groups).** Section-index scrubbers,
   ...
   persistence (LuauUI ships `replication` and game data is the game's job),
   list-item tint, chart descriptions, `resetFocus`, `focusedObject`.

### 3.3 Examined — 127 groups

The document's real coverage. It is strongest exactly where LuauUI is strongest:
...
| 11 | **Pull-to-refresh** — [`refreshable`](https://developer.apple.com/documentation/swiftui/lists) | A downward over-drag on a collection triggers an async reload with a built-in indicator. | Leaderboards, friend lists, shop restock, server browsers. It is the touch-native idiom for "get me fresh data", and LuauUI's async story (`newResourceProvider`) already has the reload half. `grep -rn "pullToRefresh\|refreshable" src/` returns nothing. | **control + seam** (gesture arbitration against the scroller) |
...
| 28 | **Content margins for scrollers** — [`contentMargins`](https://developer.apple.com/documentation/swiftui/layout-adjustments) | Inset scroll *content* without insetting the scroller — the correct fix for a notch or home indicator over a list. LuauUI has safe-area insets at the surface root but no per-scroller content margin. | prop |
...
| 33 | **Toolbar / chrome item host** — [`toolbar`, `ToolbarItemPlacement`](https://developer.apple.com/documentation/swiftui/toolbars) | Declare items and let the framework place them in the surface's chrome band. LuauUI has the `topbarInset` / `topbarSafeInsets` env facts — the geometry without the hosting construct. | control |
...
> "a developer (or an agent) picking up LuauUI can find out, **in one read**,
...
| **Classify NA / OOS / Examined / Unexamined** | **hours — this is the whole mission** | **No.** Each verdict needs a Roblox-substrate judgement and a LuauUI-source check. This is where this audit spent its time. |
...
| LuauUI source read | `src/blueprint.luau`, `src/blueprint_schema.luau`, `src/init.luau`, `src/env/environment.luau`, `src/render/authority.luau`, `src/controls/`, plus targeted greps over all of `src/` |
