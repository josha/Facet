...
| 5 — Sponsor framework gaps | Every reusable gap runs in a Sponsor-shaped Studio gallery with deterministic fixtures for success, failure, interruption, reduced motion, preferred text, and teardown; the UI Designer reviews its interaction/adaptation/motion quality against named legacy criteria; relevant motion-design principles are generalized behind public Facet contracts; no RascalRally game policy appears in Facet |
...
| 11 — reference apps | Each clean-room reference proof runs a complete representative loop across the applicable five views and input paths; the feature ledger cites live evidence and never substitutes a fake for an unavailable host-platform surface |
