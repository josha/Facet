...
5. **A VERTICAL pan that begins on a row still reaches `onActivate` — and the fix depends on an engine premise nobody in this repo has verified** (RED-TEAM final pass, 2026-08-13, recorded not fixed). `hostedResolveAxis` sets `pending.declined = true` when a gesture resolves vertical and arms **nothing**, so the `Activated` a row's `Hit` fires at the end of a scroll pan started on that row lands as a real activation. **Measured** on the clean-room hosted list (`tests/virtual_list_row_actions.spec.luau`'s `world` + `dragDown` + `releaseActivating`, 40px rows): **1 stray activation in all four combinations** — `dy=20` (the release still inside the row rect) and `dy=90`, on **both** release orders, `engagedKey=nil` and no action fired in each. **Unchanged by `79649d6`** — pre-existing, not a regression of that round. It is left alone deliberately: suppressing it headlessly is only correct if a native `ScrollingFrame` drag does **not** cancel its child button's `Activated`, and if it *does* cancel it the stray is impossible on device and any arm we add is a suppression that eats real taps. That premise — *the engine cancels the child's Activate when the scroller takes the gesture* — is exactly what the **horizontal** branch refuses to rely on (it arms at the lock instead), nothing tests it, and no device evidence in the repo states it. **The device pass answers one question first: does a real touch pan over a hosted row fire that row's `Activated`?** Only then is the code decision (arm on the vertical resolve too, vs. leave it to the engine) taken.
