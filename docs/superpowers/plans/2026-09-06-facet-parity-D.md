...
- **No public API change. Observables byte-identical**: (1) `lune run tools/lune/text_reentry_differential <out>` in this tree vs a worktree at the parent commit, md5 **`6f14ec6b3e184809d8cef15094e6997b` (360 blocks)** — the fixture set as it has stood since `1a12fea1` (2026-09-07) — or every differing block explained. The number this constraint carried until then was `3db66551b80fe677d314f2092b0a0038` (144 blocks), and it is NOT stale: RED-TEAM C-2 found that fixture set crossed none of Plan D's thresholds and could not see a coordinate-space change at all, so the set was replaced and the old md5 remains the right answer FOR THE OLD SET — a different question, which is why the tool's own header records both. Every Plan D commit before `1a12fea1` carries the old number legitimately; (2) an oracle that does NOT share the mechanism's switch (§11 of the after-doc) — for each fast path, one spec drives a class the fast path does not serve and compares SLOW vs FAST on a byte-equal channel the fast path writes.
...
### Task D3: the wide-key settle — the drain first, then the cutoff, then the route

**Re-ordered after S2's brief (`.superpowers/sdd/2026-09-06-facet-parity-D/S2-settle-drain.md`), which corrected §8b's mechanism.** The one-word-per-round boot drain is NOT boxes moving: `text_metrics.luau`'s per-solve word collector has `COLLECT_CAP = 1024`, filled in document order, and a word that was REQUESTED but not yet ANSWERED is still `nil` in the store, so the in-flight backlog re-consumes the budget on every solve. Live, the first batch cannot be answered inline (`warmFont`/`PreloadAsync` and `GetTextBoundsAsync` yield; loop mode never lets the spawn thread resume), so with a 1,023-word backlog every solve has room for exactly ONE new word — which is §8b's "744, 724, 725, …" walk to the digit (`battle_hud` L's vocabulary is > 1,024 distinct words; the mount's truncation frontier is ~unit 710–725; the observed first backlog word is 724). Headless the fake target answers inline, so the backlog never forms (2–3 rounds). `COLLECT_CAP` appears in no test, doc or bench. And every one of those ~63 rounds is the wide-key `full` route (the learned word marks a 1,000-user key, over the cap) — a 52 ms whole-tree cold solve per word. Removing the backlog makes the `full` route fire ONCE per vocabulary instead of once per word; that is the largest single lever and it goes first.

**D3a — in-flight words stop eating the collector budget (zero renderer chars).**

**Files:**
- Modify: `src/layout/text_metrics.luau` — a process-global refcount `inFlight: { [string]: number }` keyed exactly as the collector keys (`{canon}\0{fontSize}\0{word}`); `text_metrics.noteInFlight(font, size, word, delta)`; in `wordWidth`'s collect branch: `if queue[key] == nil and (inFlight[key] or 0) == 0 and collectCount < COLLECT_CAP` — an in-flight word is neither re-requested nor charged; `text_metrics.collectTruncated()` reads a counter incremented once per solve when the collector leaves `wordWidth` with `collectCount == COLLECT_CAP` (latched per collector, so one solve counts once).
- Modify: `src/render/premeasure_round.luau` — at every write of the surface's `textInFlight[key]` (dispatch) call `noteInFlight(…, +1)`; at every clear (a delivered answer, a `markUnmeasurable`, and `dispose`) call `noteInFlight(…, −1)`. The refcount is what makes a process-global set dispose-safe: a surface that dies with words in flight decrements them, and a second surface waiting on the same word keeps its count. Its header's one-way claim ("nothing but adapter/env/stats crosses") gains `text_metrics` as a named dep — check `tests/premeasure_round_seam.spec.luau`'s dep pin and move it WITH the mechanism.
- Modify: `src/render/render_stats.luau` — `stats.textCollectTruncated` published in `publish` from `text_metrics.collectTruncated()` (render_stats is uncapped; the renderer gets ZERO chars).
- Create: `tests/text_collect_inflight.spec.luau` (register) — a fixture with > 1,024 distinct words (1,200 `UI.Text` rows) on the fake target with delivery DEFERRED (drive `adapter.answer`/`syncAnswer` by hand, never inline): at HEAD, `pendingTextRequests()` per round reads n=1 in document order after the first batch (S2's OWED experiment — the red arm is the demonstration); after: the second round asks for the whole remaining tail (`#pending == vocabulary − 1024`), `textMeasureBatches` HEAD (read off the run) → 2, `textCollectTruncated` 1 at HEAD on the mount solve → still 1 (the cap is real; the fix is what happens NEXT), 0 on every later solve. Oracle: a THIRD arm with the whole vocabulary `setMeasured` before mount — rects and `text_metrics.measuredDump()` byte-equal on all three arms after settle. Plus `text_round_reentry.spec`'s three observables, and the differential md5 with every differing block explained (the batch count is a recorded observable and MOVES by design — say which blocks and why).
- RR: `facet_text_settle_contract.spec` (`solves=2 notifies=2`, re-read off the run) + `./run-tests.sh` ≥ 3,599; the results-screen canary counts batches live.
- Live (the number this exists for): `battle_hud` L fresh VM, loop mode `samples=100` — `stepP95Ms` 890 ms (§8b) and the drain-cap line; `drainDeferrals` 0. Then the three `[S]` wide-key rows in the settled probe (they should be UNCHANGED per step but reached ~63x less often in the boot window).

**D3b — re-sweep the cutoff after D1 + D3a.** `lune run tools/profile/t18_cutoff 1000 7` in a worktree at the D3a head (with the `_G.T18A_CAP` temporary edit re-applied in that worktree only, never committed) vs at `b315dc34`. The PUSH arm paid `memoPlan`'s whole-tree walk on every push solve (S2 §4a: the 1.9x at 33 % is "the same measure work plus a failed skip protocol", and the protocol's biggest term was `memoPlan` + the cacheKey); read the new crossover band and move `max(64, lastLayoutNodes // 8)` to the measured band only if the band moved — the constant is a measurement, never a guess (renderer chars: same-length constant or seam first). Then the wide-key classes live.

**D3c — the route for a key still over the cap (S2 route (a)).** Profile-gated on D3b leaving `battle_hud updateItem-facing [S]` / `war_room updateItem-power [S]` over target. Between `take` and the closure build: for each pushed path re-run `text_metrics.measure` ALONE at the offer the previous solve used (`ctx.offers`, written on both uncached and hit paths since T6) and compare with the stored rect; build `dirty_closure.of` from ONLY the leaves whose box moved (`always`/fit nodes bypass the probe and are pushed unconditionally); an empty set is an EMPTY closure (already a plan, not a refusal). Right-AND-fast: every label drawing a corrected token is still re-measured; only the ones the correction provably did not move are dropped. Traps: run the probe with the collector CLOSED (it would widen `textRequests`); same `lineHeight`/`lineLimit` inputs as the solver. Channel it can change: `textStates`/`compact`/`textFacts`/`fitCuts` for a dropped leaf whose `truncated`/`compact` verdict moved without its box — the 800-tree differential fuzz comparing those three channels between the probe route and a forced-full route is the oracle, plus a fixture with a label exactly on a truncation boundary at the corrected width. Files: `src/render/text_index.luau` (`take` returns `probe = leaves` over the cap), `src/render/settle_solve.luau` (NEW seam: the settle-solve branch of `solveAndApply` and the `textIndex.apply` call move out of the renderer — own commit FIRST, byte-identical), `src/layout/solver.luau` ≤ +300. Counters: `lastTextProbeMeasures`, `lastTextDirtyPaths` (the MOVED set), `lastMeasured` on the settle step 14,263 → moved × depth. Alternative to measure beside it (S2 route (c)): a STRUCTURAL solve from the pushed paths' LCA (`structural_scope` already runs a partial solve from a boundary) — build the two-line form too and let the arms choose.
