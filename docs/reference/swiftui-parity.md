# SwiftUI ↔ Facet: what Facet has, what it doesn't

Facet is a declarative UI framework for Roblox. SwiftUI is the most complete
...
Facet can find out in one read whether the thing they need exists, exists with
caveats, or does not exist at all — and, where Facet and SwiftUI genuinely
differ, why.
...
of thing is Facet, and how does its thinking differ from SwiftUI's?* — without
...
differences trace back to one fact about the platform: **Facet does not own the
...
So Facet keeps a manifest — `src/render/authority.luau` — that names, for every
...
things, Facet raises at construction and names the alternative, instead of
...
Facet splits the same job into two pieces that never touch each other:
...
- **The instance tree is flat by default, with four registered exceptions.**
  Every node is positioned by an absolute rectangle the solver computed, and an
  ordinary object is not parented to its container at all. A `ScrollView`, a
  ...
  authored `scale`/`rotation` reaches children it actually has, each register as
  a real engine parent instead, because the engine can then carry something down
  that one subtree the framework would otherwise compute per descendant. Outside
  those four a container moving still carries nothing inside it, which is why
  animating a *move* still has to accumulate offsets down the subtree while
  animating a *size* still must not (§8.1) — but inside a registered host the
  accumulation now stops one hop earlier, at that host's own move.
...
### What Facet deliberately is not
...
- **There is no assistive-technology bridge of any kind.** Nothing talks to a
  screen reader. A blind player cannot use a Facet interface. That is a hole,
  not a decision (§7).
...
and two surfaces painting over each other. Facet's answer to "this looks nearly
...
1. **A control that works on only some input devices is Partial at best.** Facet
...
2. **Nothing in Facet has been confirmed on physical hardware.** Every
...
SwiftUI parity says nothing about whether Facet is worth using *on this
platform*. The standing bar from the director is that Facet must do **more**
...
Facet's terms, in SwiftUI terms where an analogue exists:

| Facet term | What it is |
|---|---|
| **Blueprint** | The tree of plain Lua tables describing what should be on screen — Facet's equivalent of a SwiftUI `View` body. `UI.VStack{ UI.Text{...} }`. |
...
| **Composite** | A shipped, exported, tested control assembled from primitives — `Facet.newSlider`. The opposite of a "recipe" the consumer writes by hand. |
...
| **Control contract** (`src/class_contract.luau`) | A per-control *declaration*: focus role, which semantic actions it consumes, its minimum hit-target size (enforced by the renderer), and a readable accessibility summary. |

---

## 2. The honest summary

Facet's reactive core, layout solver, motion system, theming system, and tooling
...
- There is **no assistive-technology bridge at all**. Nothing in Facet talks to
  a screen reader; a blind player cannot use a Facet interface. This is the
...
Roblox-specific or Facet-specific capabilities the comparison cannot score in
either direction.
...
This is Facet's strongest area. Where SwiftUI invalidates a view and recomputes
...
diffing as the mechanism ([SW-04]) — Facet tracks dependencies per *value*: a
...
| `.task` — async work scoped to view lifetime ([SW-07]) | **Partial**, with stronger cancellation than SwiftUI | `Facet.newResourceProvider` gives scope-owned handles and generation-counter stale-completion rejection: a slow request that returns after its owner changed identity is discarded rather than applied. Bounded, spaced retry. SwiftUI's own guarantee is that it cancels the task when the view goes away or changes identity ([SW-07]); what is added here is refusing a stale result that *completed*. Not a `.task`-shaped modifier, though | `src/async/resources.luau` |
...
| `ForEach(id:)` / `.id()` — identity and structural diffing ([SW-09]) | **Covered**, closer to `ForEach` than to whole-tree diffing | Adds, removes, and moves only; duplicate keys are a hard error. A row removed and re-added *while its exit animation is still playing* resumes the same mounted subtree, scope, and instances rather than remounting. **Apple documents no behaviour here** — nothing on the `ForEach` page says what happens to an element removed and re-added mid-transition ([SW-09]) — so read that as a Facet guarantee, not as a win over a documented rule | `src/mount.luau` |
...
Facet's solver is a headless, testable measure-then-arrange pass over the
...
- **There is no `LayoutOrder` analogue.** Declaration order is the only order.
  ...
  in the tree; in Facet you reorder the `children` array, or the keys a
...
- One pattern in `src/controls/row_actions.luau` is worth knowing about but is a
  *recipe built on an existing seam*, not a framework primitive: a per-row height
  override signal driven by a physics spring, to animate a row collapsing to
  zero. Its second recipe — reading `syncGeometry` on the scroll cadence to keep
  a floating menu anchored to a moving row — **is a primitive now**:
  `presenter.presentAnchored` does it for any panel, and `row_actions`' own menu
  placement asks the same shared solver rather than carrying its own copy.
...
Facet and the engine's own `UIListLayout` + `UIFlexItem`. The right question is
...
| Native capability | Facet | Verdict |
|---|---|---|
...
| `UIFlexMode` `Shrink`/`Fill` + `ShrinkRatio` | `shrinkWeight` (proportional, `weight × basis`) **inside** `layoutPriority` tiers (ordered, lowest-first) | **Superset.** Native gives a flat per-item ratio with no tiering concept; CSS gives only the proportional level and SwiftUI only the ordered one ([SW-13]). Facet composes both, for one sort |
...
So Facet has **two** windowed collection controls, and they share their
arithmetic:
...
`align-content` to invent**. Facet's `align` / `lineAlign` / `distribute` map
...
`space-between` *between lines* has no native counterpart and Facet does not add
one.
...
Facet expresses that with the composition machinery of §4, not a second system.
...
rest of that strip free. Every safe-area primitive Facet already had expresses
...
Facet's conformance registry holds **59 rows: 32 composite classes and 27 leaf
primitives**. Nineteen of those rows are interactive, and **all nineteen carry an
automated four-input proof and also prove the device-idiom axis** — the two
counts `lune run tools/lune/check_registration_cli` prints. That is far short of
SwiftUI's catalog in breadth and ahead of it in per-control rigour.

| SwiftUI item | Verdict | What Facet has | Evidence |
|---|---|---|---|
| `.swipeActions` — secondary actions on a row ([SW-37]) | **Covered** | `Facet.newRowActions` / `Facet.newRowActionsCoordinator`, a general construct that wraps *any* row content, plus turnkey `spec.rowActions` on **both** `Table` and `newVirtualList`. The defaults match Apple's: trailing edge, full-swipe on ([SW-37]). Being *general* rather than list-bound used to be a Facet lead and no longer is — SwiftUI's own swipe actions now apply "to a row in a list or container", meaning scroll views, stacks, grids and custom layouts ([SW-38]). The one place Facet is still ahead of the modifier itself is platform reach: Apple ships `swipeActions` on iOS, macOS and watchOS and **not on tvOS** ([SW-37]). Detail and remaining gaps in §5.1 | `src/controls/row_actions.luau`; `src/init.luau`; `tests/row_actions.spec.luau`, `tests/row_actions_input.spec.luau` |
...
| `Picker` (`.segmented` and `.inline`) ([SW-41]) | **Partial** (upgraded 2026-08-16, navigation-and-menus D6) | One adaptive composite replaces both styles: `picker.resolvePresentation(optionCount, sizeClass, longestLabel)` chooses segmented vs inline from option count, screen size class, and label length — never from a device name. `presentation` is `"automatic" \| "segmented" \| "inline"`. **D6 added three properties, no new construct**: `Option.icon` with a group-wide `iconOnly` (SwiftUI's `.labelStyle(.iconOnly)` applied to the segments — `label` stays the semantic name, and a half-iconned group is refused per the HIG); `axis = "y"`, a real vertical *pill* (a content-sized rail floored at 44 px, distinct from the inline row list and reported apart from it by `dump().verticalPill`); and `indicator`, which now **defaults to the sliding pill** on the segmented presentation. **A segmented picker used as a tab bar IS `TabView`**, which owns the content subtree, the screen-restructuring placement and nesting, and which composes this control for its own strip — so there is exactly one segmented row in the framework. Still Partial: no `.menu` / `.navigationLink` / `.wheel` / `.palette` presentation, and `PickerStyle` itself is Missing by decision (§6.1) | `src/controls/picker.luau`; registry |
...
| `AsyncImage` — a picture that loads later, with a placeholder in the meantime ([SW-132]) | **Covered** | `Facet.newAsyncImage`. The load runs through `newResourceProvider`, so the composite is engine-free and testable: `state` is `"pending" \| "ready" \| "failed"`, the placeholder is painted while pending, and **a failure keeps the placeholder** rather than drawing a broken-image glyph. That last rule is Apple's own, arrived at independently — SwiftUI's page says that if an `Image` placeholder "doesn't load, SwiftUI doesn't show anything as a placeholder and doesn't report an error" ([SW-132]). Two things Facet adds that Apple has no counterpart for: a required `scope` (the provider handle is released with its owner, and a stale completion is rejected by generation counter rather than applied to a reused node), and per-call-site `retry = { count, delaySeconds }`. Two it lacks: there is no `phase`-closure initializer — the three states are the composite's own layers, not a consumer-authored view per phase — and no image cache (Apple's is transport-level from the 27.0 releases, [SW-132]); Roblox's content system does its own caching underneath, which this framework neither controls nor observes | `src/controls/async_image.luau`; `src/init.luau`; `tests/async_image.spec.luau` |
| `Label` (title + icon) | **Partial** | Real composite with the presentation resolution SwiftUI's `LabelStyle` provides: `presentation: "titleAndIcon" \| "titleOnly" \| "iconOnly"`, default `titleAndIcon`, and `iconOnly` degrades safely to `titleOnly` when no icon resolves. **Apple documents no behaviour here** for the iconless case: `LabelStyle.iconOnly` documents only that the title survives for VoiceOver ([SW-46]) and says nothing about what is painted, so the honest claim is that Facet defines the case and Apple's documentation does not. **Its `title` is a static string, not bindable** — a known follow-on with no shipped screen asking for it. It also does **not** compose into `newPopupButton`: that control builds its trigger and its rows as `UI.Button{ label = … }` with a flat string, never a Label blueprint | `src/controls/label.luau`; `src/controls/popup_button.luau` |
...
| Discrete-segment level indicator — SwiftUI has none, and AppKit's is macOS-only ([SW-170], [SW-173]) | **Covered**, and this is the one row where Facet unifies on every platform what Apple splits across two frameworks and omits from SwiftUI entirely | `Facet.newLevelPicker` — `count` discrete segments over one owner-held Signal, one focus stop, tap/scrub/Adjust on all four input classes. **The absence is enumerable, not argued**: `GaugeStyle`'s built-in styles are exactly `automatic`, `circular`, `linear`, `linearCapacity`, `accessoryCircular`, `accessoryCircularCapacity`, `accessoryLinear`, `accessoryLinearCapacity` ([SW-170]), and none is segmented — SwiftUI's `Gauge` is a CONTINUOUS fill, “a current level of a value in relation to a specified finite capacity, very much like a fuel gauge in an automobile” ([SW-171]), whose capacity style “displays a fill that stops at the value's location on the path” ([SW-172]). The segmented control the reference screenshot shows is AppKit's `NSLevelIndicator`, “a visual representation of a level or quantity, using discrete values” ([SW-174]), whose `discreteCapacity` style “displays discrete segments that indicate the capacity of something” and whose `rating` style “indicates a rank, such as a star ranking display” ([SW-173]) — **macOS only, in the framework SwiftUI replaced**. Facet has one control for both: `segment = "bar" \| "glyph" \| "image"`. **`tint = { filled, empty }` is Apple's palette rendering** — `.symbolRenderingMode(.palette)` plus the two-argument `.foregroundStyle`, which “use[s] the palette rendering mode when you apply this modifier” ([SW-176]) — with each half independently optional and defaulted from theme ROLES only where a bar has no other way to say “filled”. **The ± buttons are deliberately NOT in it**: they compose as a separate `newStepper` over the same Signal, which is Apple's own rule — “a stepper sits next to a field that displays its current value, because the stepper itself doesn't display a value” ([SW-177]). A `count` above 11 raises a `diagnostics()` note rather than a refusal, because the HIG states the hazard (“a large value range can make the segments of a discrete capacity indicator too small to be useful”, [SW-172]) and states no number | `src/controls/level_picker.luau`; `tests/level_picker.spec.luau`; fixture `examples/gallery/scenarios/level_picker.luau` |
| Star-rating strip ([SW-175]) | **Covered** — no SwiftUI standard-library counterpart ([SW-10]); AppKit has one as an `NSLevelIndicator` style ([SW-173]) | `newRating`: a single focus stop that supports tap, scrub, and keyboard/gamepad adjust, and cancels back to its prior value if the pointer is lost mid-scrub. Since 2026-08-16 it is a **thin preset over `newLevelPicker`** (count 5, glyph segments, the star pair) rather than a second implementation — same closed spec, same `starSize`, same `facet-rating-dump/1`. It already matched the HIG's rounding rule, “a rating indicator doesn't display partial symbols; it rounds the value to display complete symbols only” ([SW-175]) | `src/controls/rating.luau`; `src/controls/level_picker.luau` |
...
| `.contextMenu` ([SW-50]) | **Covered** | `Facet.newMenu` — a freestanding action menu attachable to **any** blueprint. Items are verbs (`{ id, label, icon?, role?, enabled?, onSelect }`), plus a divider and a submenu item, and the panel presents on `presenter.presentAnchored`. Apple's documented trigger — *"touch and hold in iOS or iPadOS"* ([SW-50]) — is honoured by **consuming** the normalized gesture layer that used to be the blocker (§5.2), and it is one of five: primary activate, pointer secondary click, touch long-press, a gamepad button and the keyboard context key. **Submenus nest with no structural cap** (matching the API, which documents none) with a diagnostic past one level (matching the HIG, which advises one); under the touch `sheet` presentation a submenu **replaces** the contents with a Back row rather than stacking panels. A menu whose every item is disabled still opens, per Apple's own rule. Cancel closes one level, tap-away closes all, Right/Left enter and leave in document order. The divergence worth knowing: SwiftUI's `contextMenu` shows a *preview* of the source view on iOS; Facet shows none, and the source stays where it is | `src/controls/menu.luau`; `tests/menu.spec.luau` |
| `.help(_:)` — a tooltip / help tag ([SW-149], [SW-150]) | **Covered** | `UI.Button{ help = "…" }` (also on `Text`, `Toggle` and `TextField`) — one sentence about what a view does, PULLED by the player: pointer hover after a 0.45 s dwell, keyboard/gamepad on focus. **Nothing appears on touch, and that is the specification rather than a gap.** Apple's own scope is the desktop help tag plus an accessibility hint ([SW-149]) and a pointer-hover interaction ([SW-150]); there is no touch trigger and no long-press binding anywhere in the API, and there is none here either — touch long-press is the disclosure plate's, which is the only touch route to a truncated label's own value (§5.2). The consequence is enforced rather than promised: `text_audit.helpRoutes` reports a help string the screen paints nowhere else (waivable) **and** help that no live gesture reaches (not waivable — the half `clippedEssential` structurally cannot do, because a declaration cannot answer whether a gesture still exists). The plate is presented through `presentAnchored`'s CHROME mode: no stack, no focus scope, no input context, so it cannot take navigation from the control it describes. A truncated `disclose` label on the same engagement outranks it. Divergences: no accessibility-hint channel exists in Facet, so the hint half of `.help` has no analogue; the plate carries D1's arrow tail since the 2026-08-16 device review (Apple's help tag has none, but a stem answers "which control is this about", which a leading-aligned plate under a row of same-sized buttons otherwise leaves open; the coach mark below is distinguished by who raises it and that it retires, never by the arrow) | `src/blueprint_schema.luau` (`HELP`); `src/present/help_plate.luau`; `presenter.help()` / `presenter.helpDeclarations()`; `src/layout/text_audit.luau` (`helpRoutes`); `tests/help.spec.luau` |
| TipKit coach marks — `popoverTip(_:arrowEdge:action:)` ([SW-151], [SW-152]) | **Covered** | `Facet.newCallout` — the app-PUSHED plate with an arrow tail, raised “when the tip becomes eligible for display” ([SW-151]) rather than pulled by a hover. Content is a **blueprint**, not a string. Eligibility is TipKit's actual contribution and every rule is a READ of caller state: `seen` (once per player), `sessions` + `afterSessions`, `featureUsed`. Invalidation is permanent — feature used, player dismissed, or explicit. **Persistence is the caller's**: Facet has no save layer and does not grow one, so the construct reports through a REQUIRED `onRetire(reason)` and writes nothing. It never blocks (not modal, no focus trap, the tap-away catcher's non-consuming mode, so the control the arrow points at stays operable), and at most one is on screen — a queue over the shipped toast scheduler, not a second one. Apple's warning is carried verbatim into `api.md`: *“Use tips sparingly… Don't use tips to guide people through your app, or for advertising and promotion purposes.”* ([SW-152]). Divergence: TipKit's `Tips.configure` datastore has no analogue and deliberately never will | `src/controls/callout.luau`; `src/present/callout_queue.luau`; `presenter.presentCallout` / `presenter.callouts()`; `tests/callout.spec.luau` |
...
`Facet.newRowActions` and `Facet.newRowActionsCoordinator` are standalone
...
### 5.2 `.contextMenu` — closed 2026-08-16, by consuming what already shipped

This section used to read *"why it is Missing"*, and its own conclusion named the
fix: **"the adaptation layer is built, tested, and exported, and nothing consumes
it as a trigger."** `Facet.newMenu` (navigation-and-menus D2) is that consumer.

The *menu* half was always proven — row actions render exactly that kind of
action list — and it is now literally the same code: `src/controls/menu_recipe
.luau` holds the one row recipe and the one presentation rule, and `newMenu`,
`newPopupButton` and `newRowActions`' action menu all render from it.

The *trigger* half is the part that is new, and it is a consumption rather than a
construction. `src/input/touch_gestures.luau` normalizes the engine's own
recognizers and `adapter.setTouchGestureHandlers` delivers them; `newMenu` reads
`kind == "longPress"` off that stream through `controller.attachContextGestures`.
The one channel that did **not** exist is the pointer's secondary button, which
`Button` never claimed (its `Activated` fires for the primary button and touch
only) — `adapter.setSecondaryActivate` is the new, optional capability for it,
and an adapter without it degrades by name: the menu still opens on the other
four triggers.

**Who owns long-press, and why the answer is not "the new construct wins".**
Touch long-press was already claimed by the full-value disclosure plate, and it
is the **only** touch route to a truncated label's full value: `UI.Text{ reveal =
"auto" }` is opt-in, `UI.Text`-only, at most one strip per presentation and
disabled entirely under reduced motion, and a disclosing `Toggle` label or
`Table` cell has no marquee even in principle. `text_audit` cannot see the
difference either — it checks the *declaration*, never the reachability. So the
binding was kept, and `newMenu` carves out only its own trigger subtree
(`contribution.contextTriggers` → `presenter._discloseLongPress`). Everywhere
else the accessibility route is exactly what it was. `help` (D3a) binds no
gesture at all, matching Apple, and `tests/menu.spec.luau` fails if a third
construct starts competing for the word.
...
The durable difference is the opposite of the strength: Facet lets a *theme*
...
**Facet will not add `ButtonStyle` / `ToggleStyle` / `PickerStyle` /
...
| Apple's protocol | What you are actually trying to change | Facet route |
|---|---|---|
...
| `PickerStyle` | which presentation | `picker.resolvePresentation(...)` decides segmented vs inline from option count, size class and label length. Force it with `presentation = "segmented" \| "inline"`; `"automatic"` is the default. The segmented presentation's *axis* (`"x"` / `"y"`, the vertical rail) and its selection paint (`indicator`, defaulting to the sliding pill) are properties too — and `.tabs` is not a picker style here at all: it is `TabView`, which composes this control's strip |
...
Facet's is a ladder of **derived floors**: each rung is a smaller width the text
...
comments. A blind player cannot use a Facet interface. There is also no
...
**four independent implementations** that share almost nothing. And Facet's
...
| `.accessibilityAction` — custom accessibility actions ([SW-83]) | **Missing** | Nothing. Apple's exists so that "assistive technologies, such as the VoiceOver, [can] interact with the view by invoking the action" ([SW-83]) — which is the layer Facet has none of | — |
| A control's accessibility description (`accessibilityLabel`, [SW-82]) | **Partial** — prose only | Every control declares an `accessibility` string on its control contract (§1), but it is typed as a human-readable summary and the only consumer asserts it is non-empty. It reaches no platform API and no assistive technology | `src/class_contract.luau`; `tests/controls_conformance.spec.luau` |
...
1. **A control says what its press MEANT.** Not "buzz", not "play a click" — a
   ...
   **is a semantic bus event, and Facet plays nothing**.
...
(`games/RascalRally/code/src/client/FacetSponsor/PlayFlow.luau`).
...
Facet's version covers Roblox's two haptic-capable classes of device.
...
as `Facet_ActivationFeedback` on the materialized button. The haptics adapter
...
### 7.2 The platform prerequisite: Facet needs the Input Action System
...
`ContextActionService`. Facet claims keys through the newer one, `InputContext`.
...
> beat a Facet `InputContext` at priority **10000**. With the camera unbound the
> same Facet claim worked immediately. A sinking legacy binding consumes a key
...
Facet uses, and priority means what it says. Be precise about the evidence
...
**Nothing in Facet enforces this, deliberately.** The framework ships opt-in
...
Facet's motion system is authoritative and opinionated. Springs are declared
...
([SW-100]); the refusal is Facet's design choice, not a gap in Apple's. An
...
| SwiftUI capability | Verdict | What Facet has | Evidence |
|---|---|---|---|
| `withAnimation` — implicit write⇄interpolation coupling ([SW-143]) | **Covered** over the solver's whole output — position **and** size — and over the authored presentation triple (`opacity`, `scale`, `rotation`) | `presenter.withAnimation(class, fn)`. Apple's page is silent on which properties animate ([SW-143]); the rule lives on `Animatable`, which states that SwiftUI "reads the old and new `animatableData` values, then interpolates between them over successive frames" ([SW-144]). Facet's answer to "which values" is exact, and §8.1 states it | `src/present/presenter.luau`; `src/render/renderer.luau` (`installAnimationRecords`); `src/render/presentation.luau`; `tests/with_animation.spec.luau`; `tests/animation_precedence.spec.luau`; `tests/authored_presentation.spec.luau`; `examples/gallery/scenarios/with_animation.luau` |
| `.animation(_:value:)` — implicit animation attached to a VIEW rather than to a mutation ([SW-148]) | **Missing** | Every animation in Facet is declared at the mutation site or by a control that owns its own motion. There is no modifier that says "animate this subtree whenever *that* value changes" — Apple's returns "a view that applies `animation` to this view whenever `value` changes" ([SW-148]). The gap is a *declaration site*, not a capability: `withAnimation` reaches the same result when the author owns the write, and cannot when the write is somebody else's | zero occurrences |
...
could. Facet's presentation channel had no authored prop in it at all — its
...
Facet's instance tree is flat by default, so an ordinary container's move
carries nothing inside it and every descendant must re-add its ancestors'
offsets, until the walk reaches a node that IS a real engine parent — a
`ScrollView`, a `clipChildren` host, a fade group, or (since
[ADR-0032](../adr/ADR-0032-nested-instance-tree.md) Decision 4) a container with
its own authored `scale`/`rotation` — where that host's own move already carries
its children and the accumulation stops one hop early. The size half **does
not** accumulate at all, and it is the same fact reaching the opposite
conclusion: a container's growth carries nothing inside it either, whether or
not it is a host, and there that means a child must keep its own solved size
while the box around it opens. Records are therefore installed at animation
*roots* for position and per node for size, and a `UI.Text` inside a growing
card neither drifts nor stretches. The authored deltas are likewise per-node and
do not accumulate in the framework: a fade group genuinely *is* its children's
instance parent, so a descendant that also faded on its own would fade twice —
and a `scale`/`rotation` host is exactly the same shape, confirmed live: a
hosted child's own `Rotation` stays `0.0` and it grows no `UIScale` of its own
even while it visibly rotates and scales, so the ENGINE composed the term once,
not the framework twice.
...
of *animatable values* ([SW-144]). So the decision is Facet's. Animating a size
...
`frame()` instead" ([SW-147]). Facet keeps that exactly.
...
Facet presents *surfaces*: screens, modals, popovers, toasts, and a couple of
...
**One term used throughout this section: priority band.** Input in Facet is
...
| `.popover` / transient panel ([SW-114]) | **Covered** | `presenter.presentAnchored(panel, opts)` — an arbitrary panel positioned against a **source view's screen rect**, with `arrowEdge`'s analogue (`anchor.edge`, `top`/`bottom`/`leading`/`trailing`), an alignment along that edge, a safe-area **flip** that never lands somewhere worse, an along-edge **shift**, and an arrow **tail** that suppresses itself when the shift carries it off the source. The placement is a pure function (`src/layout/anchor_placement.luau`) shared with the disclosure plate and `newRowActions`' menu, so there is exactly one answer. It reuses the presenter's stack, focus scope, layering and tap-away catcher; the catcher's non-consuming mode means a tap-away can close the panel *and* still reach the control underneath. The **content** half closed with D2: `Facet.newMenu` is the freestanding, arbitrary-content consumer of this seam (`newPopupButton`, a value picker, was the only popover-shaped control before it), `help` (D3a) is its CHROME-mode consumer — a layer with no stack, no focus scope and no input context, which is what a decoration that must not take the next arrow press needs — and `Facet.newCallout` (D3b) is the app-pushed one, tail and all. Adaptation is the divergence worth knowing: on iPhone, SwiftUI's popovers adapt into sheets ([SW-114]); Facet's presentation choice is `resolvePresentation`'s, made from option count, size class and live input | `presenter.presentAnchored`; `src/present/anchored.luau`; `src/layout/anchor_placement.luau`; `presenter.syncPopupCatcher` |
| `.swipeActions` / `.contextMenu` as a secondary-action container ([SW-37], [SW-50]) | **Partial** | `Facet.newRowActions` — a real construct, not a recipe, with the gaps named in §5.1. The `contextMenu` half is no longer missing (§5.2): `Facet.newMenu` ships, and `newRowActions`' own action menu renders from the same `menu_recipe`. What keeps this row Partial is that a row does not yet *declare* a context menu as a property — a caller composes the two | `src/controls/row_actions.luau`; `src/controls/menu.luau`; `src/controls/menu_recipe.luau` |
...
| `TabView` ([SW-153]) and `.tabViewStyle(.sidebarAdaptable)` ([SW-154]) | **Covered** (2026-08-16, navigation-and-menus D5) | `Facet.newTabView` — a tab strip plus the pages behind it. `selection` is the caller's Signal; `tabs` are `{ id, label?, icon?, badge?, content }` where `content` is a FACTORY, which is what makes a tab lazy. **The strip IS `newPicker`** (§5 `Picker`), so the segments, icons, 44 px floor, sliding indicator and authoring lints are one implementation rather than two, and `dump().strip` nests the picker's own dump. Adaptation is stronger than Apple's on the one axis that matters here: `.sidebarAdaptable` is documented as adapting *"to each platform"*, while `placement = "automatic"` reads `adaptive.navPlacement` — size class, height class, distance profile and live input class — so a windowed desktop pane and a tablet get the answer their SHAPE earns rather than the one their platform name implies. **Nesting** is explicit where SwiftUI leaves it to `TabSection`: an inner TabView never claims the app-level placement (`dump().placementSource` reports `policy` / `declared` / `nested`), because `content` is a factory this control invokes and a TabView built during that invocation is inside one. **Divergences, both deliberate:** tab state is EVICTED, not retained — switching disposes the previous subtree through `UI.When`'s own branch scope, and anything that must survive lives in a caller-owned Signal outside the construct (SwiftUI retains); and there is no `.page` style, no `TabSection`, and no `tabViewBottomAccessory` | `src/controls/tab_view.luau`; `tests/tab_view.spec.luau`; consumed by `examples/reference/p4_foyer` |
...
Facet has serious performance instrumentation: **20** named production-shaped
...
**Nesting was priced rather than assumed free** ([ADR-0032](../adr/ADR-0032-nested-instance-tree.md),
headless, regression signal). A second elision baseline — a 20-row list, 142
creates / 61 elided / 81 GuiObjects, **43.0 %** — was re-measured after
registering authored `scale`/`rotation` containers as real instance parents:
142 / 41 / 101, **28.9 %**, **+20 instances** — exactly the cost of the 20 move
boundaries that fixture declares, pinned exactly rather than to a floor
(`tools/lune/check_elision_census`). A container that declares neither prop is
untouched, still at 43.0 %. The lookup that decides which container is a host
also got cheaper in the same round: `hostFor` now walks the candidate path's
own separators instead of scanning every registered host, measured at 20,000
lookups against 4,001 hosts — **4.9485 s scanning versus 0.0077 s walking, 643×**.

| Capability | Verdict | What Facet has | Evidence |
|---|---|---|---|
...
| Xcode Instruments equivalent | **Partial** — headless only | Headless percentile timing with versioned budgets. No on-device, symbolicated, UI-specific profiler. For the record on the other side: Processor Trace and the CPU Counters instrument shipped in **Xcode 26** ([SW-121]); Xcode 27 added the Swift Executors instrument — tracks for the cooperative thread pool, the main actor and custom executors — and a Hitches metric that replaces the Organizer's Scrolling metric ([SW-122]). Facet has a counterpart to none of them | `tools/perf.sh`, `tools/bench.sh` |
...
This is where Facet most clearly optimizes for something SwiftUI does not:
...
Swift 6 — Luau cannot provide it, so Facet's answer is a fast, comprehensive
runtime/test-time layer instead.

| SwiftUI / Xcode capability | Verdict | What Facet has | Evidence |
|---|---|---|---|
...
| Scriptable in-Studio verification | **Covered** | A `FacetScenarioAPI` folder of BindableFunctions lets an external driver run scenarios inside a live Studio session | `examples/gallery/scenarios/runner.luau` |
...
| `#Preview` — live, resizable, hot-reloading in-editor preview | **Missing**, mitigated | No in-editor live preview exists for Facet. Mitigated by deterministic dumps, the reference-app corpus, scripted Studio drives, and the showcase place — but all of those are batch, not interactive, where Xcode's previews are "dynamic, interactive" in the canvas ([SW-123]). Xcode 27's addition here is a **Resizable Canvas mode** for iOS previews, "arbitrarily sized containers" ([SW-122]). Facet's device matrix is the scripted analogue | — |
...
are not gaps in Facet; they are operating-system features with nothing on the
...
secondary-action/swipe model yet." That predates `Facet.newRowActions` and is
...
**Nothing in Facet has been confirmed on physical hardware.** Every four-input
...
| Facet baseline | Source only: `src/blueprint.luau`, `src/blueprint_schema.luau`, `src/init.luau`, `src/row_capability.luau`, `src/virtual_extents.luau`, `src/controls/`, `src/layout/`, `src/render/`, `src/present/`, `src/client/`, `src/motion/`, `src/themes/`, `src/tokens/`, `src/input/`, `src/focus/`, `src/env/`, plus `tests/conformance/controls_registry.luau` |
...
- **Silence is recorded as silence.** Where Apple documents nothing, the row says
  **Apple documents no behaviour here** and the claim above it is restated as
  Facet's own rather than as a comparison. [SW-10] is the shared citation for
...
| **SW-140** | [`hidden()`](https://developer.apple.com/documentation/swiftui/view/hidden()) | “Hides this view unconditionally.” “Hidden views are invisible and can’t receive or respond to interactions. However, they do remain in the view hierarchy and affect layout.” “If you want to conditionally include a view in the view hierarchy, use an if statement instead” | iOS 13+, iPadOS 13+, Mac Catalyst 13+, macOS 10.15+, tvOS 13+, watchOS 6+, visionOS 1+. There is **no** `hidden(_:)` overload taking a Boolean — the unconditional signature is the whole API, which is why the Facet prop being bindable is a widening rather than a copy | 2026-08-13 |
...
| **SW-149** | [`help(_:)`](https://developer.apple.com/documentation/swiftui/view/help(_:)-9lm7l) | “Adding help to a view configures the view’s accessibility hint and its help tag (also called a tooltip) in macOS or visionOS.” | iOS/iPadOS/Mac Catalyst/tvOS 14+, macOS 11+, watchOS 7+, visionOS 1+ — the modifier compiles everywhere and the visible tag exists only on the desktop platforms | 2026-08-16 |
| **SW-150** | [`UIToolTipInteraction`](https://developer.apple.com/documentation/uikit/uitooltipinteraction) | “An interaction object that makes it possible to show a tooltip when hovering a pointer over a view or control.” | iOS/iPadOS/Mac Catalyst 15+, visionOS 1+. **Apple documents no behaviour here** for touch: there is no touch trigger and no long-press binding on the page | 2026-08-16 |
| **SW-151** | [`popoverTip(_:arrowEdge:action:)`](https://developer.apple.com/documentation/swiftui/view/popovertip(_:arrowedge:action:)) | “Presents a popover tip on the modified view… present a tip as a popover when the tip becomes eligible for display.” | iOS 17+, macOS 14+, tvOS 17+, visionOS 1+ | 2026-08-16 |
| **SW-152** | [TipKit](https://developer.apple.com/documentation/tipkit) | “Use TipKit to show contextual tips that highlight new, interesting, or unused features people haven’t discovered on their own yet… Use tips sparingly… Don’t use tips to guide people through your app, or for advertising and promotion purposes.” Its `Tip` protocol is “A type that sets a tip’s content, as well as the conditions for when it displays.” | iOS 17+, macOS 14+, tvOS 17+, watchOS 10+, visionOS 1+ | 2026-08-16 |
| **SW-153** | [`TabView`](https://developer.apple.com/documentation/swiftui/tabview) | “A view that switches between multiple child views using interactive user interface elements.” | iOS/iPadOS/Mac Catalyst 13+, macOS 10.15+, tvOS 13+, watchOS 7+, visionOS 1+ | 2026-08-16 |
| **SW-154** | [`TabViewStyle.sidebarAdaptable`](https://developer.apple.com/documentation/swiftui/tabviewstyle/sidebaradaptable) | “A tab bar style that adapts to each platform.” The `TabView` page adds: “On iPadOS, tab sections appear in both the sidebar and the tab bar. On iOS and the horizontally compact size class on iPadOS, secondary tabs appear in the tab bar.” | iOS/iPadOS/Mac Catalyst 18+, macOS 15+, tvOS 18+, visionOS 2+ (no watchOS) | 2026-08-16 |
| **SW-170** | [`GaugeStyle`](https://developer.apple.com/documentation/swiftui/gaugestyle) | **Apple documents no behaviour here** for a DISCRETE-SEGMENT indicator. This is an ABSENCE finding and it is enumerable rather than argued: the page's topic sections list the built-in styles exhaustively, and they are exactly `automatic`, `circular`, `linear`, `linearCapacity`, `accessoryCircular`, `accessoryCircularCapacity`, `accessoryLinear`, `accessoryLinearCapacity` (plus the protocol's own `Body`, `Configuration` and “makeBody(configuration:)”). None of them is segmented | iOS 16+, iPadOS 16+, Mac Catalyst 16+, macOS 13+, watchOS 7+, visionOS 1+ — **no tvOS**. Read from the JSON twin, the route this section documents | 2026-08-16 |
| **SW-171** | [`Gauge`](https://developer.apple.com/documentation/swiftui/gauge) | “A gauge is a view that shows a current level of a value in relation to a specified finite capacity, very much like a fuel gauge in an automobile.” | iOS 16+, iPadOS 16+, Mac Catalyst 16+, macOS 13+, watchOS 7+, visionOS 1+ — **no tvOS**. The sentence is what separates it from a level picker: a fuel gauge is CONTINUOUS | 2026-08-16 |
| **SW-172** | [HIG — Gauges](https://developer.apple.com/design/human-interface-guidelines/gauges) | “a gauge that uses the capacity style displays a fill that stops at the value's location on the path.” And, on a large range: “A large value range can make the segments of a discrete capacity indicator too small to be useful.” | Design guidance, no per-platform table. The second sentence is guidance and not a limit — Apple states no number — which is why Facet's high-count rule is a diagnostic rather than a refusal | 2026-08-16 |
| **SW-173** | [`NSLevelIndicator.Style`](https://developer.apple.com/documentation/appkit/nslevelindicator/style) | “A style that displays discrete segments that indicate the capacity of something, such as an audio level.” (`discreteCapacity`) and “A style that indicates a rank, such as a star ranking display.” (`rating`) | **AppKit, macOS only** — the page carries no other platform and no introduced-in version. This is the row that makes the parity claim precise: the control in the reference screenshot exists in Apple's world, on exactly one platform, in the framework SwiftUI replaced | 2026-08-16 |
| **SW-174** | [`NSLevelIndicator`](https://developer.apple.com/documentation/appkit/nslevelindicator) | “A visual representation of a level or quantity, using discrete values.” | **AppKit, macOS only** | 2026-08-16 |
| **SW-175** | [HIG — Rating indicators](https://developer.apple.com/design/human-interface-guidelines/rating-indicators) | “A rating indicator uses a series of horizontally arranged graphical symbols — by default, stars — to communicate a ranking level.” and “A rating indicator doesn't display partial symbols; it rounds the value to display complete symbols only.” | Design guidance, no per-platform table. The second sentence is the rounding rule `newRating` has always implemented | 2026-08-16 |
| **SW-176** | [`foregroundStyle(_:_:)`](https://developer.apple.com/documentation/swiftui/view/foregroundstyle(_:_:)) | “Symbol images within the view hierarchy use the palette rendering mode when you apply this modifier, if you don't explicitly specify another mode.” | iOS 15+, iPadOS 15+, Mac Catalyst 15+, macOS 12+, tvOS 15+, watchOS 8+, visionOS 1+. The two-argument form plus `.symbolRenderingMode(.palette)` is how Apple colours a filled and an unfilled symbol differently | 2026-08-16 |
| **SW-177** | [HIG — Steppers](https://developer.apple.com/design/human-interface-guidelines/steppers) | “A stepper sits next to a field that displays its current value, because the stepper itself doesn't display a value.” | Design guidance, no per-platform table | 2026-08-16 |
