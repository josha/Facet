...
LuauUI's conformance registry holds **59 rows: 32 composite classes and 27 leaf
...
it as a trigger."** `LuauUI.newMenu` (navigation-and-menus D2) is that consumer.
...
LuauUI's instance tree is flat by default, so an ordinary container's move
