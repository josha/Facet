...
| SwiftUI capability | Verdict | What LuauUI has | Evidence |
|---|---|---|---|
| `HStack`/`VStack` core (weighted, aligned, with margins) | **Covered** | Weighted flexbox distribution, `align`, `margin`, shrink negotiation (next row), per-child `lineAlign` (Roblox's `ItemLineAlignment`), and `distribute` — `start`/`center`/`end`/`spaceBetween`/`spaceAround`/`spaceEvenly`, which is Roblox's `UIFlexAlignment` plus whole-group centring, and which SwiftUI itself has no prop for (it needs hand-placed `Spacer`s) | `src/layout/solver.luau` stack measure + arrange branches; `tests/stack_distribution.spec.luau` |
| `layoutPriority` — who shrinks first when over-committed | **Covered, and wider than SwiftUI's** | Shipped 2026-08-12 (parity round 2 §2.4). `layoutPriority` is the outer sort — tiers consumed lowest-first, SwiftUI's model — and `shrinkWeight` is the inner one, proportional to weight x basis inside a tier, which is CSS's `flex-shrink` and Roblox's `UIFlexMode` and which SwiftUI has no equivalent of. Both default to inert (`shrinkWeight = 0` = `Enum.UIFlexMode.None`), so nothing shipped moved. Floors are `minMax.min` -> a text node's longest word -> `0`. The pass runs in BOTH solver passes: arrange negotiates against the real rect, and the measure branch runs the same negotiation whenever the OFFER is already too small (added 2026-08-12, closing the measure-versus-arrange seam the arrange-only version shipped with), so a squeezed label's extra wrapped lines are reserved rather than painted outside the row. `Composition`'s `rank` remains adjacent but not equivalent: it drops or degrades whole screen regions, it does not negotiate sizes inside one stack | `solver.luau` `shrinkStack`/`absorbTier`, called from the measure branch's PASS 1.5 and from arrange; `tests/stack_distribution.spec.luau` |
| `.frame(alignment:)` on ONE child of a stack — SwiftUI's route to per-child cross alignment | **Covered** | `lineAlign` (`start`/`center`/`end`/`stretch`) is a shared box prop, so a `Text`, `Image`, `Box` or `Spacer` can align itself in its parent's line. It outranks the container's `align` and a nested stack's own `align`, which is what disentangled the one word that used to mean both jobs | `solver.luau` stack arrange branch; `tests/stack_distribution.spec.luau` |
...
| `GridRow` / per-column widths / `gridCellColumns` (spanning) | **Covered** | Shipped 2026-08-13 (parity round 2 §2.2). `UI.GridRow` is a solver primitive, and a `UI.Grid` whose children are all rows switches to row mode: column *n* is as wide as the widest natural cell in column *n* across every row (SwiftUI's rule), against the flow grid's one shared width. `gridSpan` is SwiftUI's `gridCellColumns` — a spanning cell contributes to no single column's maximum and is fitted to the columns it covers plus the gaps between them. **The mode is selected by the children, never by a prop**, and a mix of rows and loose cells is an authoring error the solve reports, keeping the flow reading rather than guessing. A Grid with no `GridRow` child is byte-identical to what it always was, pinned on both sides. Not yet covered: SwiftUI's per-row `alignment:` and `gridCellAnchor` (a cell aligns with the shared `alignH`/`alignV`), and `gridCellUnsizedAxis` | `solver.luau` `gridColumnPlan`/`gridRowHeight` + the `gridrow` branches; `tests/grid_row.spec.luau`; `games/RascalRally/code/tests/luauui_grid_row_contract.spec.luau` |
...
| `containerRelativeFrame` | **Covered** | Shipped 2026-08-13 (parity round 2 §2.5). `UI.containerRelativeFrame(bp, { axis, fraction })` or the paging form `{ axis, count, span?, spacing? }`, whose arithmetic is SwiftUI's verbatim. The earlier "composable via `percent`" reading was **wrong about the ruler**: `percent` resolves against the immediate parent's offer, so a wrapper between the view and its scroller resizes it, and on a scroller's own axis (offered `math.huge`) it cannot express anything at all. This resolves against the nearest ancestor viewport — a `ScrollView`'s content viewport, else the surface root — which is what a paged carousel needs. Not yet covered: SwiftUI's multi-axis form (`[.horizontal, .vertical]`) and the `alignment:`/closure variants | `solver.luau` `containerRelativeSize` + `ctx.containerW/H`; `tests/container_relative_frame.spec.luau` |
...
| `LazyVStack` / `LazyHStack` (as `VirtualList`) | **Partial** | Windowed rendering of a long collection **on either axis** (`axis = "y" \| "x"`, construction-only), with a configurable gap and a focus policy keyed by item identity or index. **Neither SwiftUI name ships as a constructor** (game-director decision, 2026-08-12): they would have to be sugar over this virtualizer, which demands a uniform item extent, an explicit `key` and an explicit `cell` — a constructor wearing SwiftUI's name over that would be a parity claim the code does not honor (§14). The named divergences: **uniform `itemExtent` only** (variable extents are a recorded gap with two candidate designs and no implementation — `docs/plans/swiftui-parity-round2.md` §2.3), no pinned section headers, no fling/inertia, no scrollbar. Row actions are vertical-only, because on a horizontal list the tray's reveal swipe *is* the scroll gesture | `src/controls/virtual_list.luau` |
...
| `ProgressView` (indeterminate / spinner) | **Covered** | Selected by `value = nil`, SwiftUI's own rule — no second flag that could disagree with the value. `presentation = "bar" \| "spinner"`; the spinner is a ring of dots rather than an arc, because the blueprint has no rotation or trim channel and this round adds no styling system. **Reduced motion is the deliberate opposite of the usual call**: the indicator is `kind = "informational"`, so it keeps advancing on the motion authority's quantized 250 ms tick instead of freezing — a frozen spinner and a hung process look identical | `src/controls/progress_view.luau`; `tests/display_controls.spec.luau` (both RM policies); `docs/plans/swiftui-parity-round2.md` §3.1 |
...
| `Table.onPrimaryAction` | **Covered** | "Open this row", the verb that is not selection. Four-input: pointer double-click, keyboard `Return`, gamepad **A**, and — a deliberate divergence from the pointer idiom — touch taps an ALREADY-SELECTED row (any tap when `selection = "none"`), because a blanket single tap would make every selection open a row and a double-*tap* is not a touch idiom | `src/controls/table.luau`; `tests/table_input.spec.luau`; registry row `Table` |
| `Table` / `List` with selection and reordering | **Partial** | Reorderable rows, selection, per-cell rendering via `cellFor`, and swipe actions via `spec.rowActions` — leading/trailing trays, per-edge full-swipe, keyboard Delete/Backspace, gamepad menu. **Modifier-click multi-select IS shipped** (Shift-click ranges, Cmd/Ctrl-click toggles, anchor-tracked) — the claim that it was not came from a stale file header, now fixed. **No virtualization**; column resize still remounts every row | `src/controls/table.luau`; `tests/table.spec.luau` |
...
| Alignment guides (`.alignmentGuide` / custom `AlignmentID`) and baseline alignment | **Missing** | §4 |
