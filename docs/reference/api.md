...
| `scale` | every rendered class | paint-only uniform scale about the node's centre; `1` is unscaled. **It changes nothing the solver sees** — the layout box, the tap target and the focus order are all the unscaled ones — which is Apple's rule for `scaleEffect` too; to change a box, change `width`/`height`. It **multiplies** with any scale the framework is applying (a motion pop, an enter transition). Reactive and animatable. **One known gap, booked and not yet fixed:** a `Button`'s press dip and this share the engine's single `UIScale` per object, and the dip's recovery tweens back to an absolute `1` — so an authored `scale` on a Button is lost from the first release onward. Put the scale on a wrapper (`UI.ZStack{ scale = … }`) until that lands; ADR-0026's Consequences carry the fix |
