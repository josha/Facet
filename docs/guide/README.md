...
Facet is a Roblox user-interface library written entirely in Luau. It has two
...
1. **Its decisions are headlessly testable; its live mechanisms are Roblox-native.**
   ...
   make Facet a portable framework for other engines. The decision layer runs
...
2. **You describe the interface; you do not build it.** You hand Facet a
   plain-data description of the screen you want. Facet figures out which real
...
- **Learn one, know them all.** Controls are built one way
  (`build(Facet, core, spec)` → `{ blueprint, dump, dispose }`), callbacks are
...
| [`12-performance-lab.md`](12-performance-lab.md) | The performance lab place: rebuilding and opening it without Rojo, publishing it privately yourself, the nine deterministic workloads and the selectors/ramp/emergency-stop/clean-capture controls that drive them, the nine MicroProfiler phase scopes and how to read them through LibMP, capturing on a low-end Android device and what conditions to record, when two captures are comparable at all, and how to return to idle and prove nothing is retained. |
| [`13-theme-catalog.md`](13-theme-catalog.md) | The shelf of ready-made looks: Studio Neutral is built in and every other theme is an optional package that builds to its own `build/themes/<Name>.rbxm`. What each of the eight looks like in one screenshot's worth of words, what it does to your spacing, rows and type, the two-route install (insert the artifact or require the module, then one `theme_controller.install` call), the touch/pointer pair you hand to `selectBy` — and an honest cost line: the artifact bytes, the art you upload under your own account, and the runtime decoration instances a layered package really creates. |
...
Facet.newEnvironment(..) -- per-device facts (screen size, input type, ...)
...
Facet.composition        -- the pure declared-content arrangement decision (UI.Composition)
Facet.replication        -- adapters for server-owned state
Facet.tokens             -- the design-token compiler
Facet.themes             -- theme packages + the effective metric snapshot
Facet.renderer           -- the low-level render driver (rarely called directly)
Facet.DEPRECATIONS       -- the retiring-surface ledger: what is on its way out,
...
`Facet.DEPRECATIONS` rather than assuming it is empty — it is generated plus
...
Facet is designed so an agent or a new maintainer can change it without relying on
...
- The repository has named headless performance scenes with percentile and regression
  ...
  measurement slots are still empty. Do not describe Facet as low-end-phone,
...
[`Facet execution contract`](../plans/agent-execution-contract.md). It defines which
