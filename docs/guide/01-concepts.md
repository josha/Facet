...
Those three primitives, plus `conditions`, are the whole adaptation model. The
individual problems they solve — a row that is too tight, a card that should be
half the visible width, a list that has to scroll cheaply — each have a short
recipe in [chapter 15](15-adaptive-recipes.md).
