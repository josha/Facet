...
| **render** | `render/renderer.luau`, `render/layout_node.luau`, `render/presentation_channel.luau`, `render/presentation.luau`, `render/authority.luau`, `render/style_lint.luau`, `render/target_contract.luau` | Walks the mounted graph, runs the solver, and drives a **render-target adapter** to create/update/remove real objects. Also enforces the property-authority rule (below). Three of these are seams of the renderer, split out when `renderer.luau` reached Roblox's 200,000-char `Script.Source` write cap: `layout_node` is the **measure seam** (a mounted node becomes a `solver.Node`, plus the button-text grammar it shares with the paint seam), `presentation_channel` is the per-surface **motion write surface** (`transform`/`transparency`, the authored `opacity`/`scale`/`rotation` triple and the `withAnimation` records, composed through one write site), and `presentation` is that channel's pure arithmetic. The public contracts did not move: `renderer.attach`'s controller and `renderer.compactForm`/`drawnButtonText` are still on `renderer`. |
