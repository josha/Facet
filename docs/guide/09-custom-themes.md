...
chapter replaces that look entirely: font, type scale, spacing, control
heights, corners, insets, and ornate nine-slice chrome — **without editing a
single screen**.

You build a **theme package**: one frozen, versioned, plain-data table. It
carries no code and no component trees. You can inspect it, diff it,
serialize it, and hand it to another project. Everything in this chapter goes
through the public `Facet.themes` surface and the client-side
`theme_controller`. Nothing here requires an internal import.

The worked example is **Fantasy Parchment**, the repository's proof theme.
Its source is [`examples/themes/fantasy_parchment.luau`](../../examples/themes/fantasy_parchment.luau).
Every code block below is quoted from it. Read that file alongside this
...
- **A theme may not move anything.** A theme rule can write paint (fills, text
  ...
  The compiler catches this when the package compiles, before it reaches a
  device. See
...
Never start from a blank table. Start from the library's own package instead.
When a future Facet version adds a core role, your package inherits it through
a diagnosed upgrade, instead of silently missing it:
...
Rejections you will meet in practice:

- a missing core role
- an unknown field (with a "did you mean")
- a colour pair below the 4.5:1 contrast floor
- a target size under 44 px
- a nine-slice recipe with no declared `fallback`, or one naming an asset the
  package never declared
- an incompatible `schemaVersion`
...
number. `textSize = "body"` and `gap = "m"` are legal wherever a number is.
...
**A view that names no size still gets your role.** Each text-bearing class has
an INTRINSIC role: `Text` and `Toggle` draw at `body`; `Button` and `TextField`
draw at `control`. Facet resolves that role at the measure seam, *and writes it
again at the paint seam*. So moving `control` moves both the box the solver
reserves and the size the engine draws.

Getting only half of that right is a real bug, and it shipped once. The
adapter drew a hardcoded 16 while the solver measured your `control` size. So a
package with a 13 px control role reserved 20 px for a one-word button label,
...
the DataModel. It is named **`FacetTheme <id>`** — for Parchment,
...
> and a picture for a panel. It stretches them correctly at any size. "Nine
> slice" just means: keep the corners crisp, and stretch only the middle. The
...
`spinner` is the one slot that takes **no art**. Here is why: the dot's colour
is not decoration — it is the loading animation. The control rewrites each
dot's tint every frame. That tint paints the dot's own plate, and any skin
would hide that plate. So a dot image would be a spinner that never moves —
indistinguishable from a hung game. The compiler refuses it, and names what
does retune a dot instead: `controls.progress.spinnerDotSize` for its size,
`radii.control` and `strokes.hairline` for its shape and edge, and
`colors.accent` for what the pulse travels through. The circular ring beside it
is retuned the same way, with `controls.progress.circularSize` and
`circularThickness`. See the API reference for the full table of what a
stroked arc can and cannot be given.
...
the *button* skin instead — a button plate stretched into a groove. So `Slider`
...
> Declare one, and Facet lifts the number above your seal and centres it on it.
> An **image** badge (an avatar headshot) is deliberately excluded. It is already
> art, and a decoration child draws *over* its parent — so a seal there would be
> a seal on top of the face.
...
label is not interactive, so construction refuses the pressable surfaces
(`control`, `accent`), the panel surface (`raised`), and the backdrops
(`base`, `scrim`). Wrap the label in a `UI.Box` if you want one of those. See
[the API reference](../reference/api.md) for the whole rule.

Two sizing rules worth following, both learned from the fixture:

- **Size the node to the seal's authored size.** Facet subtracts the recipe's
  `contentInsets` from it to give the lifted count its box, so a 28px seal with
  a 5px inset leaves 18px — comfortable for a 12px digit. The same seal on a
  24px node leaves 14px, which is *less* than the line box a 12px glyph needs,
  and the count clips.
- **Set `textAlign` yourself.** The lift centres a badge's value for you, but only
  ...
  with the alignment you gave it, and the default is start. That is how an
...
> middle is. `sliceCenter` is the rectangle inside the image that can stretch.
> Everything outside it — the corners and edges — stays pixel-exact at any
> size.
...
Otherwise opposite slices overlap and the art shears.

[`assets/themes/fantasy-parchment/provenance.md`](../../assets/themes/fantasy-parchment/provenance.md)
records the art itself, its generator, its seed, how to regenerate it, and how
to upload it and get your own content IDs. The IDs this package actually uses
are in
...
> brighten it on hover and darken it on press. That is what this section
> covers. It is also why Facet draws the resting picture very slightly dimmed:
> there has to be somewhere brighter to go. If you want the *pressed* button to be a
...
Those three numbers matter more than they look. Once the framework suppresses
the native plate (below), the tint becomes the **only** interaction feedback a
skinned control has — there is no fill left to change. That is why press is a
full third darker than rest, rather than a polite nudge.
...
**decorated node** (the button itself, not the decoration child). Each rule is
keyed on a `facet-skinned-<slot>` tag, which the adapter adds and removes along
with the decoration:
...
Facet derives each token per theme from that theme's own palette, and lifts it
toward white before it multiplies. A raw danger colour multiplied into
...
> your theme. They are always solid. A gloss or wash you put on your panels can
...
> can ask for a soft glow instead. Either way, it now takes *your* colours.
...
so one recipe tints itself differently in each theme. That is the same place,
and the same reason, `chromeGradient` lives in `extra`. `themes.define` checks the token
...
**The ten-foot profile is derived, not authored.** The design's §8.4 requires
it to be unmistakable across a room. The ring answers that by thickening. The
glow answers it with more blur and less transparency, scaled from your near
values (`chrome_slots.FOCUS_TEN_FOOT`), so you tune the look once and both
distances follow.

**Two things happen even if you never touch this.** The ring's colour now comes
from the *active theme's* accent, instead of a constant captured when the
target was built. It was blue under every package before — that is the defect
this section grew out of. And a package asking for a glow on an engine without
...
`"raised"` and `"overlay"` are the two library presets. The package compiler
validates the spec when the package compiles, against the same contract the
adapter materializes. A positive `zIndex` is a compile error, not a runtime
surprise on a device.

**Why this one is adapter-owned and not a rule.** The engine silently accepts
`::UIShadow` rules and paints *nothing* (feasibility m7). The package compiler
rejects the selector for exactly that reason. So the adapter deliberately
materializes the shadow on the node the sheet does *not* shadow, behind the
same capability probe `UI.shadow` uses. There is no rule to defeat and no dual
authority. `chromeCensus()` counts shadows (`shadows`, `shadowsPerSlot`,
`actualShadows`), so "a flat package pays zero" is a measurement.

The decoration child is `Active = false` and can never report an interaction
state of its own. So every state rule reads the **parent's** state, and
reaches the child through a child combinator
(`.facet-interactive:Hover > .facet-chrome-control`). The suite verifies all
four such selectors against a running engine.

### Two consequences you must design around

> **In plain words.** Two things change once art is involved. Both will
> confuse you if nobody warns you. First: a separate little label draws a
> skinned button's text on top of the picture, so different rules colour it.
> Second: a text field gets out of the way of its own art while you are typing
> in it.
...
the parent layer, and that is non-negotiable. So while the player edits a
field, the adapter adds `facet-chrome-editing`, the decoration flattens to the
native fill, and the lifted label hides. Design the field's *native* fallback paint to
...
has no idea an image failed — so failure detection rides the resource
provider. The recovery is tag-driven:

1. The provider reports that the asset failed. Or the target's own grace
   deadline infers this from silence. It may only do that for art the engine
   was actually asked to decode, because the engine never decodes an image it
   is not showing.
2. The adapter adds `facet-chrome-fallback` to that slot's art. It also adds
   `facet-chrome-mute` to every other art instance of it, except the condemned
   asset's own undecoded picture.
3. The package's generated `Chrome — <slot> fallback` rule paints the slot's
   flat fill, corner radius, and hairline. `Chrome — <slot> fallback hide` turns
   the muted art fully transparent.
...
attribute you had to set before pressing Play. It is a *passive* surface, so
it never steals focus from the fixture underneath it. It re-themes itself
along with everything else — if the picker still reads after a swap, the swap
worked. Set the workspace attribute `Facet_ThemePicker = false` to hide it.
The place paints through the sheet by default, so the *full* swap is what you
get. Set `Facet_ForceStyleFallback = true` to see the other arm, where a swap
moves metrics live but not the palette (§9.8). The picker warns once when it
finds itself there. The source is
[`examples/gallery/client/theme_picker.luau`](../../examples/gallery/client/theme_picker.luau).
It is an example, not library code, and it goes through the public surface
only, so a game that wants one can copy the file.
...
yours. Only the metrics region gets regenerated. There is never a second
...
Hand-edit one number inside the region, and it exits 1 naming that token. Put
it back, and it exits 0. If your metrics are not in a marker region, the CLI
treats the whole file as generator-owned and rewrites it. That is the right
behaviour for a metrics-only module — and the wrong one for a package that
also carries hand-authored art.
...
- **Installation is all-or-nothing.** Every capability check runs *before* the
  first mutation. Any of these fails with an error naming exactly what is
  missing, leaving the target and the environment untouched:

  - a schema this build does not speak
  - an unknown or unprovided capability
  - a missing root

  The capability vocabulary is closed: `themeMetrics` (always available),
  `nativeStyleSheets`, `styleTransitions`. The missing-root refusal belongs to
  native paint alone: without `StyleSheet`s there is no sheet to link and no
  root to link it at. So the install proceeds into the fallback described
  below.
...
`controller.inspect()` is the answer to "what is actually installed?" It
reports:

- package identity and stamp
- active theme, mode, and fallback state
- sheet name / seeded / migrated / stamp
- link state
- the effective snapshot
- the token attribute list
- per-font calibration state
- live connection count and swap count
...
A swap is **one transaction in one invocation**. Facet resolves the snapshot
first, so a bad theme name or a bad override throws before anything is
committed. Then `SetDerives` for paint and `env:set("themeMetrics", …)` for
geometry happen with no yield between them, and the surface re-solves
immediately. New paint and new geometry land in the **same engine frame**.
...
> forward. Short version: Facet keeps values you set. It fills in things you
> never knew about. It reports anything genuinely incompatible before the game
> runs, rather than leaving you to discover it on a device.
...
- a `perSlot` breakdown
- `actualInstances` — an independent count of what is really in the tree, so
  you can check the ledger rather than trust it
- `textLifts` — lifted labels are decoration cost too
- `fallbackSlots` and `failedAssets`

The scenario's `inspect` step returns the same census.
...
**[Chapter 10, Rich skinning](10-rich-skinning.md) walks all three rungs end
to end.** At rung 1: layers, per-state art, image bars, semantic icons, pixel
mode, and `selectBy`. At rung 3: a worked custom control. This section is the
short version.
...
Set one, and Facet paints that node's decoration from your value instead of
from the theme's `sliderThumb` recipe. It works whether or not a theme package
is installed at all, because the rung does not depend on one. Every per-view
override uses the same mechanism: the decoration is deliberately **not
tagged** with its slot. So the package's `Image` rule can never reach it, and
the adapter writes the image properties directly. A tagged instance would keep
taking the theme's value, and yours would lose on the next swap.

That is the **standing opt-out rule** made concrete: an explicit local value
opts that property out of theme changes, visibly and on purpose. That is
exactly what an explicit pixel size does when it opts out of the theme's space
steps. Everything else about
...
contract), and §9.12 below. It covers how to register namespaced roles,
declare what your control needs, and how `themes.checkCoverage` tells you
before play whether a theme covers them.
