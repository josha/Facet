...
|---|---|---|
| [`UI.Slider`](../reference/api.md#uislider) | Numeric `value`. | Required `min`/`max`, optional `step` and `format`, `label`, `enabled`, `tapToPosition`, `onChange` and `onCommit`. `step` takes a readable, so a setting beside the slider can change the grid it snaps to. Optional `thumbImage` and `trackImage` override just those art slots. `range` holds a `{ lower, upper }` band with `minGap`; `axis = "y"` runs upward; `rotation` turns the track's paint; `thumb` hides the handle at rest or always; `thumbContent` draws your own knob; `controlSize` thins the track inside a whole target. Pointer/touch drag and keyboard/gamepad adjustment share one value model. |
...
| [`UI.Picker`](../reference/api.md#uipicker) | Single `selected` cell. | `options`, `style = "automatic" | "menu" | "segmented" | "inline" | "radioGroup" | "navigationLink" | "cards"`, `label`, `query` (a searchable navigation link), `axis`, `enabled`, and selection callbacks. The automatic style is a form-row menu under touch or a pointer and a strip on a television, read from the environment. Options can carry labels, icons, badges, descriptions, `meta`, `avatar`, `sectionTitle` and availability. As a field it takes `requiredMark`, `hint`, `errorText`, `controlSize`, `appearance`, `corners` and a menu `maxHeight`; opening a menu lands on the selected row. Live options use stable identities. Segmented strips support `iconOnly` and a pill/underline selection `indicator`; radio options retain visible labels. `presentation` is the deprecated spelling of `style`. |
...
| [`UI.Vote`](../reference/api.md#uivote) | `value` of `"up"`, `"down"` or `"none"`. | Up or down over your value: `onChange` (required unless `readOnly`), your own `summary` text, `readOnly` for an informational strip, `enabled`, `controlSize`. Pressing the chosen side proposes `"none"`. It counts nothing. |
| [`UI.ColorPicker`](../reference/api.md#uicolorpicker) | A `Color3` `value`, and optionally `alpha`. | A colour well that opens a panel, or `style = "inline"`. `onChange` and `onCommit`, `modes` (swatches, spectrum, sliders, brick), your `swatches`, `draft = true` for Apply and Cancel, `allowEmpty`, and the shared field keys (`label`, `hint`, `errorText`, `controlSize`). |
| [`UI.DateTimePicker`](../reference/api.md#uidatetimepicker) | A civil date `value`, or a `range` with `selection = "range"`. | A date field that opens a calendar, or `style = "inline"`. `onChange`/`onCommit` (or the range pair), `time`, `minuteStep`, `min`/`max`, `isDateDisabled`, `weekStart`, `locale`, `clock`, range `presets` and `draft`. Values are [`Facet.civilDate`](../reference/api.md#civildate) tables in no time zone. |
...
| [`UI.Popover`](../reference/api.md#uipopover) | A short panel anchored to one control, or a sheet on a compact touch screen. Your `isPresented` fact with `onPresentedChange`, exactly one of `trigger` or `source`, a `content()` builder, placement (`edge`, `align`, `gap`, `tail`), `maxWidth`/`maxHeight` and `compact`. |
| [`UI.Dialog`](../reference/api.md#uidialog) | A modal decision that needs more than a sentence. Your `isPresented` fact with `onPresentedChange`, `title`, one scrolling `content`, `hero`, `actionLabel`, `actions` (one `default` and one `cancel` role at most), `actionLayout`, `width` and `closeButton`. Actions never close it by themselves. |
| [`UI.Sheet`](../reference/api.md#uisheet) | A resizable modal with its own scrolling body. Required `title`, `content` and a caller-owned `detent` cell; `detents`, `isPresented`, `placement`, `edge`, `width`, `header`, `hero`, pinned `actions`, `scrollPolicy` and `interactiveDismissDisabled`. |
| [`UI.Card`](../reference/api.md#uicard) | One item in a browsable collection. Required `image` and `title`, `caption`, a body `onActivate`, a `primaryAction` and a More `menu`; `reveal = "automatic"` shows the actions on engagement and `"always"` keeps them. In a `UI.VirtualGrid`, point `browseTarget` at the cell. |
| [`UI.Pagination`](../reference/api.md#uipagination) | Numbered pages of results. Your `page` with `onChange`, `pageCount` (or `hasNext`/`hasPrevious` when the count is unknown), `form = "numbers" | "arrows" | "label"`, `siblingCount`, `boundaryCount`, `showFirstLast` and `direction`. It selects pages and never fetches them. |
| [`UI.StepIndicator`](../reference/api.md#uistepindicator) | Where the player is in a workflow. `steps` (each with `id`, `label` and a `state`), your `current` step id, and `onSelect` for steps marked `navigable`. When the labels do not fit, it becomes a summary with a Steps list. |
...
## Status, identity and feedback

| Control | Use it for | Configuration that changes its behavior |
|---|---|---|
| [`UI.Avatar`](../reference/api.md#uiavatar) | One player's picture or initials. | Required `name`; one of `image`, `userId` or `key` (the last two with your `provider`); `form`, `controlSize` or `diameter`, `presence`, `backplate`, `frame`. `onActivate` adds one Button target. |
| [`UI.AvatarGroup`](../reference/api.md#uiavatargroup) | A party or team shown as faces and a count. | Required `items` (each with `id` and `name`), `provider`, `layout = "stacked" | "spread"`, `max`, `overflow`, `overflowLabel`. `onOverflow` adds the only target. |
| [`UI.Badge`](../reference/api.md#uibadge) | A short state or category caption on a plate. | `label` and/or `icon` (icon-only needs `name`), `appearance = "standard" | "status" | "utility"`, `status`, `corners`, `controlSize`, `over`. No focus stop. |
| [`UI.StatusIndicator`](../reference/api.md#uistatusindicator) | A small state mark or unread count. | `form`, `status`, `count` with `max`, `cutout`, `name`, `controlSize`. No input target. |
| [`UI.Skeleton`](../reference/api.md#uiskeleton) | A loading silhouette in the shape of the content to come. | Required `form = "box" | "line" | "circle"`, `lines`, `width`, `height`, `corners`, `controlSize`. Reduced motion removes the sweep. |
| [`UI.Notice`](../reference/api.md#uinotice) | A status message that stays in the page. | Required `message`, `title`, `severity`, `appearance`, `placement = "inline" | "affixed"`, `icon`, `link`, up to two `actions`, and `onDismiss` for a close button. It never takes focus itself. |
| [`UI.Snackbar`](../reference/api.md#uisnackbar) | A short confirmation at the bottom of the screen, one at a time. | Your `isPresented` fact with `onPresentedChange`, required `message`, `icon`, one `action`, `closeButton`, `duration` (none means it stays) and `priority`. Screen applications only. |
