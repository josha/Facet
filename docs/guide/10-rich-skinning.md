...
- a frame with corner ornaments and a title plaque
- a health bar whose track *and* fill are images
- a switch whose ON state is a different picture
- a pixel-art skin that stays nearest-neighbour crisp
- one view tree that re-skins itself when the player docks a phone to a monitor
...
> **What is proven, and what is not.** The headless suite and the
> `theme_authoring` Studio scenario cover everything in this chapter. Every
> visual claim comes from a running session. The physical phone pass and the
...
> for a fantasy frame. You want:
>
> - a wood ground
> - a gilt border over it
> - four different corner ornaments
> - a rail down each edge
> - a title board hanging off the top
>
> So a slot can name a short list of layers instead. Facet draws them back to
> front. It is a *list*, not a drawing program. Every layer is one of six kinds,
> with a fixed set of numbers you may give it.
...
moves *onto* the plaque, instead of sitting in its content rect. This is the
same lifted label §9.4 already describes, applied to the plaque's rectangle.
`textInsets` is the padding inside the plate, in px. It keeps a carved border
from eating its own title. `textInsets` is only legal alongside `text = true`.
A typo'd side name is a compile error, not padding that silently fails to
apply. The nameplate creates nothing new — it costs zero extra instances.

**`overhang` renders OUTSIDE the decorated rect. A clipping ancestor will cut
it off.** The number is published as reservation metadata
(`chrome_slots.overhangFor`), and the solver leaves room for it inside the
screen. But `ClipsDescendants` belongs to the engine, not the framework: a panel
whose plaque overhangs 20px into a `UI.ScrollView` (or any `clipChildren`
container) loses those 20px at the clip boundary. This happens silently, and
only when the panel sits at the top of the region.

Nothing detects this: the census publishes the number, but no check compares it
against an ancestor's clipping. If you use a layered slot with an overhanging
plaque inside a scroll region, do one of two things. Give the region that much
leading padding, or drop the overhang to zero for that skin.
...
It is legal on `fill`, `frame` and `tile` — the three full-bleed kinds. It is a
...
trough carries a 1px hard rim and a 1px lit bottom lip. These two lines make
...
height, at every width. This lets it follow a stretched track instead of a
fixed radius. It is legal on the same three full-bleed kinds and a compile
error on the others. **`"pill"` is the only value.** The framework materializes
a real `CanvasGroup` per masked layer, and has measured only that shape.

**They compose, and they answer different questions.** The mask decides the
silhouette. The inset decides how far off the rim the content sits. The
...
measured **~7 KB and ~18 µs each**, Studio-derated. The framework counts them
as `canvasMasks` in the chrome census, so the number is visible rather than
assumed. That is cheap for a bar. It is not cheap for a masked layer on every
row of a long list — count before you reach for it. A layer that only needs to
stop short of the corners wants an `inset`, which costs nothing.
...
census measures the real number, and that is the number to design against.
Fantasy Ornate's live census reads 60 declared layers against **139 actual
layer instances** across the gallery — roughly a 2.3× multiplier.
`adapter.chromeCensus()` breaks it down per kind, so "the ornate skin costs
this much" is a measurement rather than an impression. A flat theme still
creates exactly zero.
...
- **`barFill`** — a bar's fill is *clipped whole art* inside the adapter's
  ...
  **nothing at all**. Put the decoration on `barTrack` instead.
  `glossy_touch.luau` is the shipped precedent — its barber-pole stripe rides
  the track, over a plain sliced fill. Both are compile errors that name the
  slot, the reason and the fix.
...
An asset reference is legal in several places:

- a recipe's `asset`
- a layer's `asset`
- a per-corner override
- a bar, toggle, or stepper slot
- **the per-view props at rung 2**

In any of these, you may write either form:
...
- **Focus is not a variant state.** Focus applies to whichever node has focus,
  ...
  decoration, the ring (or the glow) hangs off that instance, not off the node.
  The node's own rect includes the padding the engine needs for its label, and a
  decoration is a child of it. So a ring on the node would leave a band of empty
  air on each side of the plate it outlines. For a layer stack, the plate is the
  back-most full-bleed layer; a corner ornament or an overhanging plaque is a
  detail *on* the plate, not the plate itself. A node with no art keeps the
  solved-rect ring exactly as before, and hit geometry never moves. The touch
  floor is the node's own size plus its invisible expander, and the focus path
  never touches either one.
- **A variant may declare `contentInsets` only if the values are identical
  across states, on each axis.** Otherwise it is a compile error naming the
  slot, the state and the axis. The reason is non-negotiable: art may change on
  hover, but **geometry may not**. A reflow under the player's finger is a bug
  you cannot tint your way out of.
...
a retail cold content delivery network (CDN), so the framework keeps the
preload rather than optimizing it away on evidence that does not exist.
...
- **`barCap`** and **`barCenter`** are fixed-size ornaments anchored to the
  **track**. That is exactly why they are separate slots: a cap baked into the
...
carries its own solid native paint (the accent), so an unskinned spinner still
reads under a flat package. And like every other value-control slot, it
refuses a gradient: a wash's alpha over a value control is what made the
slider thumb see-through.
...
`presentation = "circular"` covers two cases: the ring that fills to a value,
and the rotating one that has no value to report. Facet draws both as a
*stroke* on `UI.Path`, not as a picture. There is nothing to skin — a stroke
has only a colour and a thickness. It takes its colour from the ordinary style
roles: the arc is `accent`, and the unfilled capacity ring behind it is
`secondary`. It takes its size from two theme metrics,
`controls.progress.circularSize` and `controls.progress.circularThickness`.
Both are **optional** in a package. Leave them out, and the snapshot fills them
from your own `space` scale, so an existing package gains a correctly-sized
...
- the fill art keeps the same width, origin, `Image`, `ScaleType` and
  `SliceCenter`
- the window reveals 0 / 12 / 598 / 1183 / 1195 px of it
- the caps and the crown do not move by a pixel, and the adapter writes
...
*centre line*. So a 40x40 cap on an 8 px track paints 20 px past each end and
16 px past each edge. Those extents are derived — never authored twice — by
`chrome_slots.barReservation`, published on the snapshot as
`chromeOutsets.barTrack`. The framework adds them to the bar's **margin**, so
the solver moves the label, the readout and the rows above and below out of the
way. (Contrast `contentInsets`, which reserves *inward* for the node's own
content.) A package that declares neither ornament reserves nothing, and its
bar row is byte-identical to a flat one. Before this existed, an ornate cap
painted straight over the label at one end and the value at the other. The
reservation is what makes a big ornament a design choice rather than a
collision.

### Art has a minimum height, and that is a METRIC

> **In plain words.** A painted rail is not a coloured line. It has a carved
> edge at the top and another at the bottom. If you draw it 6 px tall, those
> two edges land on top of each other and the rail turns to mush. So when your
...
`facet-slot-barTrack` / `facet-slot-barFill` rules paint the two solved nodes
solid — the same own-paint family the slider rail and thumb belong to. And as
with the slider, a **gradient may never target them**: a wash's alpha would
make the value show through the glass, so declaring one is a compile error.

That solid paint is the *flat* guarantee. A bar node can be skinned three
ways: by your recipe, by a layer stack, or by a per-view image override on a
package that declares nothing for the slot. The moment that happens, the
matching `Skinned — <slot>` rule removes the plate, the corner and the
hairline underneath it — exactly as it does for a button. The art is the
element.
...
**ON is a different picture, not a tint.** It rides the toggle's value tag. The
knob's **travel stays solver-owned**: art never moves geometry, so a skinned
switch and a flat one travel the same 20 px. A package that declares neither
slot gets exactly today's palette-true switch (§9.4's `togglePalette` contract
does not change).
...
not declare it. So a theme published before this slot existed keeps painting
its steppers exactly as it always did. A new slot must never make an old theme
worse.
...
- **Size rides the snapshot.** `metrics.iconSizes = { small, medium, large }`. A
  metric package resizes every icon at once, and an icon drawn at its authored
...
- **No icon means a real character, never tofu.** The framework owns an
  ...
  your theme's own font. This is not theoretical — the measurement is on file.
  The old disclosure carets were `U+25B8` and `U+25BE`, two GEOMETRIC-SHAPES
  characters that **Michroma does not contain**. A live glyph probe ran in the
  running client, drawing them in that package's own resolved face. It measured
  both at the tofu placeholder's fixed 30x48 advance, while the ASCII
  characters rendered as themselves. The failure was a present-in-some-faces
  character, not a private-use codepoint. A package may name any font, so
  ASCII is the rule, and `themes.isSafeGlyph` is falsifiable.
...
1. **Every image rule carries `ResampleMode = Pixelated`.** The census publishes
   both counts, so `pixelatedRules == imageRules` is an assertion rather than a
   hope (19 == 19 live).
2. **`SliceScale` locks to integers at compile.** A fractional slice scale under
   `Pixelated` stays hard-edged but *unevenly sized*. It still looks crisp in a
   screenshot, but it is wrong on the grid — the worst of the three failure
   modes.
3. **The snapshot's lengths snap UP to multiples of `pixelUnit`.** They only
   snap up, never down, so a 44 px target floor becomes 48 at unit 6 rather
   than 42. Durations and ratios are not lengths, so the snapshot leaves them
   alone.

A non-pixel package stays byte-unaffected. A test proves this by comparing a
package's metric dump before a pixel package installs, and again after it
swaps away.
...
then exactly one design pixel. Any target size replicates that one uniform
colour, so it cannot produce an uneven cell. And the general slice rule still
applies: **`2 × border` must be strictly less than the shorter side of the
art**, or opposite slices overlap and the picture shears.

**The "nothing here" idiom.** The per-state grammar requires a `default`.
Sometimes a layer should exist *only* in one state — for example, Pixel
Quest's ruby corner jewels appear on the selected row and nowhere else. For
that case, point `default` at a fully transparent PNG (`pixel_blank`, 24×24 of
nothing). Saying "no art in this state" out loud beats shipping a dimmed
ornament nobody asked for.
...
strings end to end: the engine coerces a string onto an `ImageContent`
property. So the framework has no materialization machinery at all, and an
exported package stays serializable. Two packages identical except for which
spelling they use compile to the same 81 rules and paint byte-identically.
...
- **The right package installs at install time.** It resolves from the live
  ...
  phone rows.
- **A profile change swaps once, after it settles** (0.25 s by default). A
  hybrid device that flaps produces one swap per settlement, not a swap storm.
  A flap that returns home swaps nothing at all.
...
forced flips: 110 of 112 mount-identity entries were byte-identical, focus
held, and no view-tree node changed identity. Real geometry did move — the
action button went 202×63 → 202×45, and the stepper row went 203×55 → 181×45.
The two entries that changed are the adapter's own bar clip pair, which the
framework rebuilds by design when the fill art changes.
...
controls' own chrome — slider rail and thumb, bar track and fill — keeps its
solid paint guarantee. Those nodes must stay visible under a flat theme too.

**Layer paint is rule-owned from birth.** The adapter never writes a layer's
paint at create time. An explicit write made before a rule matches would
survive and defeat the rule permanently. Every layer's plain `.Image` reads
...
> **In plain words.** One slider in your game is special. You do not need a
> new theme, and you do not need a new control. You hand *that* slider its
> own picture. Everything else about it stays on the theme.
...
swap. `dump().skinRung` reports `"theme"` or `"view"`, so a probe can see which
is in force. A bare content URI carries no slice geometry, so the framework
paints an overridden image whole.

That is the **standing opt-out rule** made concrete. An explicit local value
opts that property out of theme changes, visibly and on purpose — exactly as
an explicit pixel size opts out of the theme's space steps.
...
modifiers. One sentence covers how they compose with the theme: **the view
modifier wins on that node, and the theme's own treatments stay untouched
everywhere else.**
...
They read outside-in, in the order you wrote them, and each one returns a new
blueprint. All three are pure paint: none of them enters the layout solver, so
adding a wash never moves a pixel.
...
and compile to phantom `::UIGradient` rules with zero child instances. That is
right for a theme, because a rule matches a *class* of nodes. A view-level
gradient has to win on exactly one node instead, so it follows `UI.shadow`'s
architecture: bounded normalized data under the style authority. The adapter
materializes it as **one** bespoke `UIGradient` child named `FacetGradient`,
and reuses that child rather than re-creating it. That is what makes a
**package swap safe**. Measured live, the view's ramp survives the swap on the
same node object, with no second ramp stacked on it. The theme's own gradients
keep painting everywhere else.
...
- **a value control's own chrome** — `sliderTrack`, `sliderThumb`, `barTrack`,
  ...
  `chromeGradient` (§10.3), for the same measured reason. A wash's alpha makes
  the node see-through, and whatever the control draws behind it reads
...
the one composition rule that will surprise you, so here it is measured rather
than guessed. A `UIGradient` multiplies the node's OWN rendering; it does not
reach children. On a flat or gradient-based package, the node paints a real
fill, and the wash lands on it. On a package that skins that slot with art,
the image-is-the-element suppression (§10.9) has already set the node's own
`BackgroundTransparency` to 1, and moved the paint into decoration CHILDREN.
So there is nothing left for the wash to multiply, and the art wins. Live, on
one unedited card carrying one `UI.gradient`, across three packages:
...
One more consequence of eager normalization is visible in the captures. The
ramp's colour TOKENS resolve when you build, against the style you built
under — not against whatever package installs later. If a wash must follow
the palette, pass explicit `{ r, g, b }` values, or rebuild the blueprint.
...
> is a boiler-pressure gauge. Write the control: bring your own pictures, and
> declare the few values you want the *theme* to own. Then ask a theme —
> before the game runs — whether it covers them.
...
**Re-theming a custom control.** A framework control re-solves on a package
swap, because it reads the snapshot. Yours can too, if you bind the values you
resolved to reactive props. Live, under two packages: on a `fantasy-ornate` →
`pixel-quest` swap, the gauge's dial moved 56 px → 48 px, and its corner
radius went 8 → 4. All 12 of its mount-identity entries stayed byte-identical,
with **no rebuild** — just one signal write. Its needle *glow* did not change
colour, because `UI.shadow` had already normalized it at build time (§10.10).
That is the seam, stated rather than hidden.

**What a control may NOT do.** Decoration slots are a closed framework
vocabulary — a control cannot invent `chrome.myThing`. What it can do is reach
the existing slots through the public `surface` prop. A `UI.Box` with
`surface = "raised"` gets your package's panel skin, layers and all — that is
why the gauge sits inside Fantasy Ornate's carved frame in the capture. A
control can also paint its own art with `UI.Image`. `UI.Image` has no slice
geometry and no tint, on purpose: those stay theme-recipe authority. **Design
the art for the authority you have.** The gauge's channel is authored
horizontally invariant, so stretching it is lossless. Its needle and end caps
are drawn at a fixed px box and never stretched at all.
